...
import {
  AVATAR_ASSETS,
  type EmployeeAvatarAssetKey,
} from './assets/avatars/avatar-manifest';
import { expertAvatarImage } from './assets/expert-avatars/expert-avatar-manifest';
...
export type EmployeeProfile = {
  ...
  avatarPreset: string;
  avatarImage: string;
  expertAvatarCategory: string;
  expertAvatarKey: string;
  onboardedAt: string;
  workStyles: string[];
...
const PRESET_AVATAR_KEYS: Record<string, EmployeeAvatarAssetKey> = {
  'service-orbit': 'service',
  'after-sales-seal': 'after-sales',
  'knowledge-node': 'knowledge',
  'commerce-compass': 'commerce',
  'ops-grid': 'ops',
...
type AvatarSource = Pick<EmployeeProfile, 'avatarKind' | 'avatarImage' | 'avatarPreset'>
  & Partial<Pick<EmployeeProfile, 'expertAvatarCategory' | 'expertAvatarKey'>>;
...
export function employeeAvatarImage(profile: AvatarSource): string {
  ...
  if (profile.expertAvatarCategory) {
    return expertAvatarImage(profile.expertAvatarCategory, profile.expertAvatarKey || profile.expertAvatarCategory);
  }
  const assetKey = PRESET_AVATAR_KEYS[profile.avatarPreset || DEFAULT_AVATAR_PRESET] || 'default';
  return AVATAR_ASSETS[assetKey];
}
...
export function normalizeProductDisplayText(value: string): string {
  return value;
}
...
export function isEmployeeUsedByCurrentUser(agent: AgentProfileRead): boolean {
  if (agent.used_by_current_user !== undefined) return agent.used_by_current_user;
  const metadata = agent.metadata || {};
...
export function canManageEmployeeAgent(
  ...
  if (agent.manageable_by_current_user !== undefined) {
    return agent.manageable_by_current_user;
  }
...
const EXPERT_SOURCE_LABELS: Record<string, string> = {
...
export function expertSourceCode(agent?: AgentProfileRead | null): string {
  return isExpertEmployee(agent)
    ? stringFromMeta(agent?.metadata || {}, 'expert_source_code').trim()
    : '';
}

export function expertSourceLabel(agent?: AgentProfileRead | null): string {
  const code = expertSourceCode(agent);
  return EXPERT_SOURCE_LABELS[code] || code;
}

export function expertCategory(agent?: AgentProfileRead | null): string {
  return isExpertEmployee(agent)
    ? stringFromMeta(agent?.metadata || {}, 'expert_category').trim()
    : '';
}

export function expertSubcategory(agent?: AgentProfileRead | null): string {
  return isExpertEmployee(agent)
    ? stringFromMeta(agent?.metadata || {}, 'expert_subcategory').trim()
    : '';
}

export function expertTags(agent?: AgentProfileRead | null): string[] {
  return isExpertEmployee(agent) ? asStringArray(agent?.metadata?.expert_tags) : [];
}

export function expertUpstreamUrl(agent?: AgentProfileRead | null): string {
  if (!isExpertEmployee(agent)) return '';
  const value = stringFromMeta(agent?.metadata || {}, 'upstream_url');
  try {
    const url = new URL(value);
    if (
      url.protocol === 'https:'
      && url.hostname === 'github.com'
      && url.pathname.startsWith('/msitarzewski/agency-agents/blob/')
    ) return url.toString();
  } catch {
    return '';
  }
...
function expertManifest(agent?: AgentProfileRead | null): Record<string, unknown> {
  const value = agent?.metadata?.expert_capability_manifest;
  return value && typeof value === 'object' && !Array.isArray(value)
    ? value as Record<string, unknown>
    : {};
}

export function expertCapabilityType(agent?: AgentProfileRead | null): 'P0' | 'P1' | 'P2' | 'P3' {
  const value = expertManifest(agent).capability_type;
  return value === 'P1' || value === 'P2' || value === 'P3' ? value : 'P0';
}

export function expertReadiness(agent?: AgentProfileRead | null): 'ready' | 'partial' | 'blocked' {
  const value = expertManifest(agent).readiness;
  return value === 'ready' || value === 'blocked' ? value : 'partial';
}

export function expertUnresolvedRequirements(agent?: AgentProfileRead | null): string[] {
  return asStringArray(expertManifest(agent).unresolved_requirements);
}

export function expertSearchText(agent?: AgentProfileRead | null): string {
  if (!isExpertEmployee(agent)) return '';
  const metadata = agent?.metadata || {};
  return [
    stringFromMeta(metadata, 'expert_name_original'),
    ...expertTags(agent),
    expertCategory(agent),
    expertSubcategory(agent),
    expertSourceLabel(agent),
    expertCapabilityType(agent),
    ...expertUnresolvedRequirements(agent),
  ].filter(Boolean).join(' ');
}
...
export function employeeProfile(agent?: AgentProfileRead | null): EmployeeProfile {
  ...
  return {
    ...
    roleName: isOverall
      ? '开放广场'
      : stringFromMeta(metadata, 'role_name')
        || (isExpertEmployee(agent) ? expertCategory(agent) : '')
        || template?.roleName
        || '待补充岗位',
    ...
    expertAvatarCategory: isOverall ? '' : expertCategory(agent),
    expertAvatarKey: isOverall
      ? ''
      : stringFromMeta(metadata, 'upstream_path')
        || stringFromMeta(metadata, 'upstream_url')
        || stringFromMeta(metadata, 'expert_name_original')
        || agent?.id
        || agent?.name
        || '',
