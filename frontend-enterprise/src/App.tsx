...
const OrganizationRolesPage = lazy(() => import("./pages/OrganizationRolesPage"));
...
function Shell({
  ...
  const [guidesCompleted, setGuidesCompleted] = useState(() =>
    Boolean(
      window.localStorage.getItem(BRAND_STORAGE_KEYS.onboardingSeen) &&
      window.localStorage.getItem(BRAND_STORAGE_KEYS.quickStartSeen),
    ),
  );
  const isMobile = useIsMobile();
  const isAdmin = isEnterpriseAdmin(auth.user);
  const governancePermissions = auth.user.governance_permission_codes || [];
  const canReadMembers = hasGovernancePermission(auth.user, "member.read");
  const canReadOrganization = (
    hasGovernancePermission(auth.user, "organization.read")
    || hasGovernancePermission(auth.user, "position.read")
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener(PRODUCT_EVENTS.quickStartCompleted, onQuickStartCompleted);
    return () =>
      window.removeEventListener(PRODUCT_EVENTS.quickStartCompleted, onQuickStartCompleted);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "gongge-enterprise-agent-scope-refresh",
      onAgentRefresh,
    );
    return () =>
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "gongge-enterprise-agent-scope-change",
      onScopeChange,
    );
    return () =>
      window.removeEventListener(
  ...
  useEffect(() => {
    ...
    window.addEventListener("gongge-enterprise-agent-create", onCreateAgent);
    return () =>
      window.removeEventListener(
        "gongge-enterprise-agent-create",
        onCreateAgent,
      );
  }, []);
  ...
  const showModelSetupNotice =
    guidesCompleted && modelConfigsLoaded && !hasUsableModelConfig;
  ...
  const selectedAgentCaption = selectedAgent
    ...
    : "-";
  function openCreateAgentModal(sourceAgentId?: string) {
    setAgentForm({
      ...EMPTY_AGENT_FORM,
      sourceMode: sourceAgentId ? "copy" : EMPTY_AGENT_FORM.sourceMode,
      copyFromAgentId:
        sourceAgentId || selectedAgentId || sourceAgents[0]?.id || "",
    });
  ...
  async function saveAgentCreateModal() {
    ...
    try {
      ...
      const copiedCount = created.copy_summary?.copied?.length ?? 0;
      const skippedCount = created.copy_summary?.skipped?.length ?? 0;
      notify.success(
        agentForm.sourceMode === "copy"
          ? `数字员工创建成功：复制 ${copiedCount} 项，按安全边界跳过 ${skippedCount} 项`
          : "数字员工创建成功",
      );
    } catch (error) {
        ...
        >
          ...
          {!isDistillRoute && !showEmployeeEmptyState && (
            <Routes>
              ...
              <Route
                ...
                element={
                  <OpenPlatformPage
                    currentUser={auth.user}
                    isAdmin={isAdmin}
                    onCopyAgent={openCreateAgentModal}
                    onLogout={onLogout}
                  />
                }
              />
              ...
              />
              <Route
                path="/enterprise/work-items"
                element={
                  <WorkItemsPage currentUser={auth.user} onLogout={onLogout} />
                }
              />
              <Route
              ...
              <Route
                ...
                element={
                  canReadOrganization ? (
                    <OrganizationPage currentUser={auth.user} onLogout={onLogout} />
                  ) : (
                    <Navigate to={EnterpriseRoute.Gallery} replace />
                  )
                }
              ...
              <Route
                ...
                element={
                  <EnterpriseInfoPage currentUser={auth.user} onLogout={onLogout} />
                }
              />
              <Route
                path="/enterprise/organization-roles"
                element={
                  canReadAuthorization ? (
                    <OrganizationRolesPage currentUser={auth.user} onLogout={onLogout} />
                  ) : (
                    <Navigate to={EnterpriseRoute.Gallery} replace />
                  )
                }
              />
              <Route
                path="/enterprise/reference-data"
                element={
                  canReadReferenceData ? (
                    <ReferenceDataPage currentUser={auth.user} onLogout={onLogout} />
                  ) : (
                    <Navigate to={EnterpriseRoute.Gallery} replace />
                  )
                }
              />
              <Route
                path="/enterprise/management-audit"
                element={
                  canReadAudit ? (
                    <ManagementAuditPage currentUser={auth.user} onLogout={onLogout} />
                  ) : (
                    <Navigate to={EnterpriseRoute.Gallery} replace />
                  )
                }
      ...
      <Dialog open={agentCreateOpen} onOpenChange={setAgentCreateOpen}>
        <DialogContent className="flex max-h-[calc(100dvh-32px)] w-[calc(100%-32px)] flex-col gap-0 overflow-hidden rounded-[16px] p-0 sm:max-w-[520px]">
          ...
          <div className="agent-editor-form min-h-0 flex-1 overflow-y-auto px-[24px] pb-[16px]">
            <div>
              <span className="mb-[6px] block">创建方式</span>
              <div className="inline-flex w-fit gap-[4px] rounded-[var(--gg-radius-control)] border border-[var(--gg-border)] bg-[var(--gg-cloud)] p-[2px]">
                {[
...
export default function App() {
  ...
  const [enterpriseContext, setEnterpriseContext] =
    useState<EnterpriseContext | null>(null);
  ...
  useEffect(() => {
    ...
    void api.get<EnterpriseContext>("/api/auth/context")
      .then((context) => {
        if (cancelled) return;
        if (context.member.tenant_id !== context.tenant.id) {
          throw new Error("企业上下文与当前成员不一致");
        }
        setEnterpriseContext(context);
        const refreshed = { token: auth.token, user: context.member };
      ...
      .catch((error) => {
        if (cancelled) return;
        if (isAuthError(error) || (error instanceof ApiError && error.status === 403)) {
          clearEnterpriseAuthSession();
          setAuth(null);
        }
        setEnterpriseContext(null);
      ...
      <BrowserRouter>
        <Routes>
          <Route
            path="/*"
            element={
              auth && !authChecked ? null : auth && enterpriseContext ? (
                <EnterpriseContextProvider value={enterpriseContext}>
