...
import { createDistillWorkspaceParams } from '@/lib/distill-workspace';
...
export default function SkillsPage({
  ...
  const load = async () => {
    ...
    try {
      ...
      const result = await api.get<SkillRead[]>(`/api/enterprise/skills?tenant_id=${getRequestTenantId()}${suffix}`);
      setRows(result);
  ...
  async function loadImportSourceSkills(sourceAgentId: string): Promise<SkillRead[]> {
    ...
    try {
      const sourceRows = await api.get<SkillRead[]>(`/api/enterprise/agents/${sourceAgentId}/skills?tenant_id=${getRequestTenantId()}`);
  ...
  function openCreate() {
    navigate(`/enterprise/skills/distill?${createDistillWorkspaceParams(agentId).toString()}`);
  }
  ...
  async function publish(row: SkillRead) {
    try {
      await api.post(`/api/enterprise/skills/${row.skill_id}/publish?tenant_id=${getRequestTenantId()}${agentQuery()}`);
      notify.success('已启用');
      await load();
    } catch (error) {
  ...
  async function archive(row: SkillRead) {
    try {
      await api.post(`/api/enterprise/skills/${row.skill_id}/archive?tenant_id=${getRequestTenantId()}${agentQuery()}`);
      notify.success('已停用');
      await load();
    } catch (error) {
  ...
  async function markDraft(row: SkillRead) {
    try {
      await api.post(`/api/enterprise/skills/${row.skill_id}/draft?tenant_id=${getRequestTenantId()}${agentQuery()}`);
  ...
  async function openVersions(row: SkillRead) {
    ...
    try {
      const result = await api.get<SkillVersionRead[]>(
        `/api/enterprise/skills/${encodeURIComponent(row.skill_id)}/versions?tenant_id=${getRequestTenantId()}${agentQuery()}`,
      );
  ...
  async function showVersionDetail(row: SkillVersionRead) {
    try {
      const result = await api.get<SkillVersionRead>(
        `/api/enterprise/skills/${encodeURIComponent(row.skill_id)}/versions/${encodeURIComponent(row.version)}?tenant_id=${getRequestTenantId()}${agentQuery()}`,
      );
  ...
  async function syncFromOverall(row: SkillRead) {
    ...
    try {
      await api.post(
        `/api/enterprise/agents/${agentId}/skills/${encodeURIComponent(row.skill_id)}/sync-from-overall?tenant_id=${getRequestTenantId()}`,
      );
  ...
  async function confirmDelete() {
    ...
    try {
      await api.delete(`/api/enterprise/skills/${row.skill_id}?tenant_id=${getRequestTenantId()}${agentQuery()}`);
  ...
  async function confirmRollback() {
    ...
    try {
      const result = await api.post<SkillRead>(
        `/api/enterprise/skills/${encodeURIComponent(row.skill_id)}/versions/${encodeURIComponent(row.version)}/rollback?tenant_id=${getRequestTenantId()}${agentQuery()}`,
      );
      notify.success(`已从 ${row.version} 派生新版本 ${result.version}`);
  ...
  async function confirmPromote() {
    ...
    try {
      await api.post(
        `/api/enterprise/agents/${agentId}/skills/${encodeURIComponent(row.skill_id)}/promote-to-overall?tenant_id=${getRequestTenantId()}`,
      );
  ...
  return (
    <div className="min-h-full box-border px-[48px] pt-[32px] pb-[43px] max-[900px]:px-[16px]" aria-busy={loading}>
      ...
      <div className="flex flex-col gap-[24px] rounded-[20px_20px_0_0] bg-white p-[18px_18px_24px_18px] shadow-[0_-4px_16px_0_rgba(0,0,0,0.05)]">
        <div className="flex flex-col gap-[18px]">
          ...
          <div className="flex flex-wrap items-center gap-[16px]">
            <label className="flex h-[34px] w-[260px] items-center gap-[8px] overflow-hidden rounded-[10px] border-[0.5px] border-[#e3e7f1] bg-white px-[12px] transition-colors focus-within:border-[var(--gg-cobalt)] max-[900px]:w-full">
      ...
      <ConfirmDialog
        ...
        description={
          rollbackTarget
            ? `系统将复制「${rollbackTarget.name}」的 ${rollbackTarget.version} 内容并发布为新版本；原版本、历史对话和反馈数据都不会被修改。`
            : ''
        }
        confirmText="回滚"
