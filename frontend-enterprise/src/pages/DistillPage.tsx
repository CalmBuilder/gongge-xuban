...
import { api, ApiError, streamGet, streamPost, getRequestTenantId } from '../api/client';
...
type SopBusinessRoleOption = { id: string; role_code: string; name: string; status: string };
type SopPermissionOption = { id: string; permission_code: string; name: string; status: string };
...
export default function DistillPage({ active = true, searchParamsOverride, currentUser, onLogout }: DistillPageProps = {}) {
  ...
  const cacheIdentity = distillCacheIdentity(skillId || '', mode, workspaceId);
  const cacheKey = `skill-distill:${getRequestTenantId()}:${activeAgentId || 'default'}:${cacheIdentity}`;
  ...
  const [sopBusinessRoles, setSopBusinessRoles] = useState<SopBusinessRoleOption[]>([]);
  const [sopPermissions, setSopPermissions] = useState<SopPermissionOption[]>([]);
  ...
  const [selectedRewriteModelId, setSelectedRewriteModelId] = useState(
    () => window.localStorage.getItem(`${DISTILL_REWRITE_MODEL_STORAGE_KEY}:${getRequestTenantId()}`) || '',
  );
  ...
  useEffect(() => {
    if (!active) return;
    const nextParams = addMissingDistillWorkspaceId(searchParams);
  ...
  useEffect(() => {
    ...
    api
      .get<SkillRead>(`/api/enterprise/skills/${encodeURIComponent(skillId)}?tenant_id=${getRequestTenantId()}${agentQuery}`)
      .then((result) => {
  ...
  useEffect(() => {
    const canReadGovernanceCatalog = currentUser?.role === 'admin';
    Promise.all([
      api.get<ToolRead[]>(`/api/enterprise/tools?tenant_id=${getRequestTenantId()}${agentQuery}`),
      canReadGovernanceCatalog
        ? api.get<SopBusinessRoleOption[]>(
          `/api/organization/business-roles?tenant_id=${getRequestTenantId()}`,
        )
        : Promise.resolve([]),
      canReadGovernanceCatalog
        ? api.get<SopPermissionOption[]>(
          `/api/organization/permission-definitions?tenant_id=${getRequestTenantId()}`,
        )
        : Promise.resolve([]),
    ])
      .then(([toolRows, roleRows, permissionRows]) => {
        setTools(toolRows);
        setSopBusinessRoles(roleRows.filter((role) => role.status === 'active'));
        setSopPermissions(permissionRows.filter((permission) => permission.status === 'active'));
      })
      .catch(() => {
        setTools([]);
        setSopBusinessRoles([]);
    ...
    api
      ...
      .then((rows) => {
        ...
        setSelectedRewriteModelId((current) => {
          ...
          if (fallback) {
            window.localStorage.setItem(`${DISTILL_REWRITE_MODEL_STORAGE_KEY}:${getRequestTenantId()}`, fallback);
          }
          return fallback;
        });
      })
  ...
  async function saveDraft() {
    ...
    try {
      ...
      if (loadedSkill) {
        ...
        try {
          savedSkill = await api.post<SkillRead>(`/api/enterprise/skills${agentOnlyQuery}`, { tenant_id: getRequestTenantId(), content: finalDraft, status: 'published' });
        } catch (error) {
  ...
  function clearDistillWorkspace() {
    ...
    const nextParams = createDistillWorkspaceParams(activeAgentId);
    const nextWorkspaceId = nextParams.get('workspace_id') || '';
    const nextRoute = `/enterprise/skills/distill?${nextParams.toString()}`;
    const nextCacheKey = `skill-distill:${getRequestTenantId()}:${activeAgentId || 'default'}:create:${nextWorkspaceId}`;
    removeDistillCache(cacheKey);
  ...
  async function rollbackPersistedOperations(
    ...
    for (const operation of toolOps) {
      try {
        await api.delete(`/api/enterprise/tools/${encodeURIComponent(String(operation.toolId))}?tenant_id=${getRequestTenantId()}${agentQuery}`);
      } catch {
    ...
    for (const operation of versionOps) {
      ...
      if (snapshot.loadedSkill) {
        ...
        if (operation.version && operation.version !== snapshot.loadedSkill.version) {
          try {
            await api.delete(
              `/api/enterprise/skills/${encodeURIComponent(skillId)}/versions/${encodeURIComponent(operation.version)}?tenant_id=${getRequestTenantId()}${agentQuery}`,
            );
          } catch {
        ...
        try {
          await api.delete(`/api/enterprise/skills/${encodeURIComponent(skillId)}?tenant_id=${getRequestTenantId()}${agentQuery}`);
        } catch {
                ...
                <div className={CHAT_ACTIONS_GROUP_CLASS}>
                  ...
                  <ModelConfigDropdown
                    ...
                    onChange={(modelId) => {
                      ...
                      window.localStorage.setItem(`${DISTILL_REWRITE_MODEL_STORAGE_KEY}:${getRequestTenantId()}`, modelId);
                    }}
        ...
        >
          ...
          {!draft ? (
            ...
            <SkillSource
              ...
              businessRoles={sopBusinessRoles}
...
function SkillSource({
  ...
  businessRoles: SopBusinessRoleOption[];
  permissions: SopPermissionOption[];
              ...
              >
                ...
                <div className={SOURCE_RENDERED_CLASS}>
                  ...
                  <div className={SOURCE_META_LIST_CLASS}>
                    ...
                    />
                    {String(step.type || '') === 'human_task' && (
                      <EditableParticipantPolicyLine
                        metadata={isRecord(step.metadata) ? step.metadata : {}}
                        businessRoles={businessRoles}
                        permissions={permissions}
                        onChange={(metadata) => editStep(index, 'metadata', metadata)}
                      />
                    )}
...
function EditableParticipantPolicyLine({
  metadata,
  businessRoles,
  permissions,
  onChange,
  ...
  const policy = isRecord(metadata.participant_policy) ? metadata.participant_policy : null;
  const selectedRoleCodes = policy ? asStringList(policy.candidate_role_codes) : [];
  const outcomeOptions = policy && Array.isArray(policy.outcome_options)
    ? policy.outcome_options.filter(isRecord).map((option) => String(option.value || '')).filter(Boolean)
    : [];
  const outcomes = outcomeOptions.length > 0
    ? outcomeOptions
    : policy ? asStringList(policy.allowed_outcomes) : [];
  const effectiveOutcomes = outcomes.length > 0 ? outcomes : ['approved', 'rejected'];
  const actionPermissions = policy && isRecord(policy.action_permissions)
    ? Object.fromEntries(Object.entries(policy.action_permissions).map(([key, value]) => [key, String(value)]))
    : {};
  const permissionOptions = [
    { value: '__none__', label: '不额外要求动作权限' },
  ...
  function updatePolicy(patch: Record<string, unknown>) {
    ...
    onChange({ ...metadata, participant_policy: { ...basePolicy, ...patch } });
  }

  function updateActionPermission(actionKey: string, permissionCode: string) {
    const nextPermissions = { ...actionPermissions };
    if (permissionCode === '__none__') delete nextPermissions[actionKey];
    else nextPermissions[actionKey] = permissionCode;
    updatePolicy({ action_permissions: nextPermissions });
  }

  return (
    <div className={SOURCE_LINE_CLASS}>
      <span className={SOURCE_KEY_CLASS}>流程参与者</span>
      <span className={SOURCE_VALUE_CLASS}>
        <EditableSourceField>
            ...
            <label className="flex items-center gap-[8px] text-[12px] font-medium text-[#24262d]">
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  if (checked === true) updatePolicy({});
                  else {
                    const nextMetadata = { ...metadata };
                    delete nextMetadata.participant_policy;
                      ...
                      <label key={role.id} className="flex items-center gap-[7px] text-[12px]">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => updatePolicy({
                            candidate_role_codes: checked === true
                              ? Array.from(new Set([...selectedRoleCodes, role.role_code])).sort()
                ...
                {[['claim', '认领'], ...effectiveOutcomes.map((outcome) => [`outcome:${outcome}`, `提交结果：${outcome}`])].map(([actionKey, label]) => (
                  <div className="grid grid-cols-[110px_minmax(0,1fr)] items-center gap-[8px]" key={actionKey}>
                    ...
                    <SourceSelect
                      ...
                      onChange={(permissionCode) => updateActionPermission(actionKey, permissionCode)}
                    />
                  </div>
                ))}
