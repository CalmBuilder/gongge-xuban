...
type Feature = {
  ...
  resourceKind?: PlazaResourceKind;
  proof: string;
};
  ...
  return (
    <main className="tutorial-doc-page">
      <section className="tutorial-doc-hero" id="intro">
        ...
        <div className="tutorial-doc-hero-map" aria-label="共格·序伴运行链路">
      ...
      <div className="tutorial-doc-shell">
        <aside className="tutorial-doc-nav" aria-label="共格·序伴单页文档目录">
      ...
      <section className="tutorial-doc-section" id="install">
        ...
        <div className="tutorial-doc-install-grid">
          <div className="tutorial-doc-command-card">
            <span>开发模式</span>
            <code>./app.sh dev</code>
            <p>构建前端并在前台挂载对话端、企业端和 API。</p>
          </div>
          <div className="tutorial-doc-command-card">
            <span>后台运行</span>
            <code>./app.sh</code>
          ...
          <div className="tutorial-doc-command-card">
            <span>查看状态</span>
            <code>./app.sh status</code>
            <p>确认端口、健康检查和日志位置。</p>
          </div>
          <div className="tutorial-doc-command-card">
            <span>停止服务</span>
            <code>./app.sh stop</code>
            <p>停止脚本托管的本地进程。</p>
          </div>
        </div>
      </section>
      ...
      <section className="tutorial-doc-section" id="core-features">
        <SectionHeading
          ...
          body="PilotDeck 把能力分成 WorkSpace、Router、Memory、Always On、Gateway；共格·序伴对应到企业数字员工场景，重点是岗位边界、知识治理、流程执行、工具调用和运营复盘。"
        />
        <div className="tutorial-doc-feature-grid">
          {FEATURES.map((feature) => (
            <article key={feature.title} className="tutorial-doc-feature">
              <span><FeatureMark feature={feature} /></span>
      ...
      <section className="tutorial-doc-section tutorial-doc-runtime" id="runtime">
        <SectionHeading
          ...
          title="一条业务请求如何被推进"
          body="共格·序伴的对话不是单轮问答。它会在路由、知识、技能、SOP、工具和回复生成之间形成可追踪执行链路。"
        />
      ...
      <section className="tutorial-doc-section" id="governance">
        ...
        <div className="tutorial-doc-governance-grid">
          {['对话日志记录每轮输入输出', 'Trace 展示路由、槽位和工具调用', '反馈分析定位失败原因', '记忆沉淀长期偏好和复盘结论', '定时任务把周期工作常驻化', '开放广场让能力复制和复用'].map((item) => (
            <span key={item}><ProductIcon name="check" />{item}</span>
          ))}
        </div>
      </section>
  ...
  -> Trace / Feedback / Memory`}</pre>
      ...
      <section className="tutorial-doc-section" id="development">
        ...
        <div className="tutorial-doc-dev-grid">
          <div><code>./app.sh status</code><span>查看当前服务状态</span></div>
          <div><code>./app.sh stop</code><span>停止单端口服务</span></div>
...
function FeatureMark({ feature }: { feature: Feature }) {
  if (feature.resourceKind) return <PlazaResourceIcon kind={feature.resourceKind} />;
  return <ProductIcon name={feature.icon} />;
}
