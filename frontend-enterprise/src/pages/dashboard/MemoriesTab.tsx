...
import type { MemoryRead, MemoryUserPageRead } from '../../types';
...
export default function MemoriesTab() {
  ...
  const [appliedFilter, setAppliedFilter] = useState<MemoryFilter>(EMPTY_FILTER);
  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(0);
  const [refreshToken, setRefreshToken] = useState(0);
  const requestIdRef = useRef(0);

  async function load(targetPage: number, next: MemoryFilter) {
    const requestId = ++requestIdRef.current;
    setLoading(true);
    try {
      const params = new URLSearchParams({ tenant_id: getRequestTenantId() });
      ...
      params.set('page_size', String(MEMORY_PAGE_SIZE));
      const result = await api.get<MemoryUserPageRead>(
        `/api/enterprise/memories/page?${params.toString()}`,
      );
      if (requestId !== requestIdRef.current) return;
      setRows(result.items);
      setTotal(result.total);
      const lastPage = Math.max(1, Math.ceil(result.total / MEMORY_PAGE_SIZE));
  ...
  }, [agentId, page, appliedFilter.username, appliedFilter.user_id, appliedFilter.q, refreshToken]);
