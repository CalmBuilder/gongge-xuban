...
export default function ConversationLogsTab() {
  ...
  const pageAgentId = useRef(agentId);
  ...
  const loadPage = useCallback(async (
    ...
    targetFilter: LogFilter,
    targetPage: number,
  ) => {
    const requestId = ++pageRequestId.current;
    setLoading(true);
    const agentQuery = targetAgentId ? `&agent_id=${encodeURIComponent(targetAgentId)}` : '';
    try {
      const result = await api.get<ConversationSessionPageRead>(
        `/api/enterprise/sessions/page?tenant_id=${getRequestTenantId()}${agentQuery}`
        + `&feedback_filter=${targetFilter}&page=${targetPage}&page_size=${FEEDBACK_PAGE_SIZE}`,
      );
      if (requestId !== pageRequestId.current) return;
      setRows(result.items.map((item) => ({
        ...item,
        downFeedback: item.down_feedback || undefined,
        upFeedback: item.up_feedback || undefined,
      })));
      setTotal(result.total);
      setSessionTotal(result.session_total);
    } catch (error) {
      if (requestId !== pageRequestId.current) return;
      notify.error(error instanceof Error ? error.message : '加载对话日志失败');
    } finally {
      if (requestId === pageRequestId.current) setLoading(false);
    }
  }, []);

  const loadMetadata = useCallback(async (targetAgentId: string) => {
    const requestId = ++metadataRequestId.current;
    const agentQuery = targetAgentId ? `&agent_id=${encodeURIComponent(targetAgentId)}` : '';
    const [summaryResult, agentResult] = await Promise.allSettled([
      api.get<FeedbackSummaryRead>(`/api/enterprise/feedback/summary?tenant_id=${getRequestTenantId()}${agentQuery}`),
    ...
    const failure = [summaryResult, agentResult].find(
  ...
  useEffect(() => {
    setPage(1);
    void loadMetadata(agentId);
    return () => {
      metadataRequestId.current += 1;
    };
  }, [agentId, loadMetadata]);

  useEffect(() => {
    const agentChanged = pageAgentId.current !== agentId;
    pageAgentId.current = agentId;
    if (agentChanged && page !== 1) return undefined;
    void loadPage(agentId, filter, page);
    return () => {
      pageRequestId.current += 1;
    };
  }, [agentId, filter, loadPage, page]);
  ...
  const openDetail = async (row: ConversationLogRow) => {
    ...
    try {
      const [sessionDetail, traces] = await Promise.all([
        api.get<EnterpriseSessionDetailRead>(`/api/enterprise/sessions/${row.id}?tenant_id=${getRequestTenantId()}`),
        api
          .get<TurnTraceRead[]>(`/api/chat/sessions/${row.id}/trace?tenant_id=${getRequestTenantId()}`)
      ...
      if (row.downFeedback || row.upFeedback) {
        try {
          feedbackDetail = await api.get<FeedbackSessionDetailRead>(
            `/api/enterprise/feedback/sessions/${row.id}?tenant_id=${getRequestTenantId()}`,
          );
        } catch {
          feedbackDetail = null;
        }
  ...
  const reanalyzeFeedback = async (feedbackId: string) => {
    ...
    try {
      await api.post(`/api/enterprise/feedback/${feedbackId}/reanalyze?tenant_id=${getRequestTenantId()}`);
      notify.success('已重新提交后台分析');
      await reloadCurrentDetail();
      await Promise.all([
        loadPage(agentId, filter, page),
        loadMetadata(agentId),
      ]);
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-wrap items-stretch gap-[20px]" aria-label="对话反馈统计">
          <StatCard value={sessionTotal} label="对话" />
...
function traceLineIcon(kind: TraceLineRead['kind']) {
  if (kind === 'skill') return <PlazaResourceIcon kind="general-skills" size="micro" />;
  if (kind === 'tool') return <PlazaResourceIcon kind="tools" size="micro" />;
  if (kind === 'knowledge') return <PlazaResourceIcon kind="knowledge" size="micro" />;
