...
type KnowledgeGovernanceDraft = {
  responsibleOrgUnitId: string;
  accessScope: 'owner' | 'organization' | 'tenant';
...
function resolveKnowledgeAgentScope(
  ...
  const canViewOverall = isEnterpriseAdmin(currentUser)
    || hasGovernancePermission(currentUser, 'knowledge.read')
    || hasGovernancePermission(currentUser, 'knowledge.manage');
  const currentAgent = rows.find((item) => item.id === currentAgentId);
  if (currentAgent) {
    if (!currentAgent.is_overall || canViewOverall) return currentAgent.id;
  }
  if (canViewOverall) return '';
...
export default function KnowledgeManagePage({ currentUser, onLogout }: KnowledgePageProps = {}) {
  ...
  const [governingKnowledgeBase, setGoverningKnowledgeBase] = useState<KnowledgeBaseRead | null>(null);
  const [governanceDraft, setGovernanceDraft] = useState<KnowledgeGovernanceDraft>({
    responsibleOrgUnitId: '',
    accessScope: 'owner',
    downloadPolicy: 'restricted',
    organizationAccess: [],
  });
  const [governanceSelectedUnit, setGovernanceSelectedUnit] = useState<OrganizationUnit | null>(null);
  const [governanceOrgNames, setGovernanceOrgNames] = useState<Record<string, string>>({});
  ...
  const [selectedSearchModelId, setSelectedSearchModelId] = useState(
    () => window.localStorage.getItem(`${KNOWLEDGE_SEARCH_MODEL_STORAGE_KEY}:${getRequestTenantId()}`) || '',
  );
  ...
  const canViewOverallKnowledge = isEnterpriseAdmin(currentUser)
    || hasGovernancePermission(currentUser, 'knowledge.read')
    || hasGovernancePermission(currentUser, 'knowledge.manage');
  const canManageCurrentScope = currentAgent
    ? canManageEmployeeAgent(currentAgent, currentUser)
    : isEnterpriseAdmin(currentUser);
  const canGovernKnowledge = isOverallAgent
    && (isEnterpriseAdmin(currentUser) || hasGovernancePermission(currentUser, 'knowledge.manage'));
    ...
    api
      ...
      .then((items) => {
        ...
        setSelectedSearchModelId((current) => {
          ...
          if (fallback) {
            window.localStorage.setItem(`${KNOWLEDGE_SEARCH_MODEL_STORAGE_KEY}:${getRequestTenantId()}`, fallback);
          }
          return fallback;
        });
      })
  ...
  async function refresh(scopedAgentId = effectiveAgentId) {
    ...
    const canViewOverall = isEnterpriseAdmin(currentUser)
      || hasGovernancePermission(currentUser, 'knowledge.read')
      || hasGovernancePermission(currentUser, 'knowledge.manage');
    if (!canViewOverall && !scopedAgentId) {
      clearKnowledgeViewState();
      return;
    }
    setLoading(true);
    try {
      const suffix = scopedAgentId
        ? `&agent_id=${encodeURIComponent(scopedAgentId)}`
        : canGovernKnowledge
          ? '&governance_view=true'
          : '';
      const [documentsResult, knowledgeBasesResult] = await Promise.allSettled([
        api.get<KnowledgeDocumentRead[]>(`/api/enterprise/knowledge/documents?tenant_id=${getRequestTenantId()}${suffix}`),
        api.get<KnowledgeBaseRead[]>(`/api/enterprise/knowledge-bases?tenant_id=${getRequestTenantId()}${suffix}`),
      ]);
      const failures: string[] = [];
      const docRows = documentsResult.status === 'fulfilled' ? documentsResult.value : documents;
      const kbRows = knowledgeBasesResult.status === 'fulfilled' ? knowledgeBasesResult.value : knowledgeBases;
      if (documentsResult.status === 'fulfilled') setDocuments(docRows);
      else failures.push('文档列表');
      if (knowledgeBasesResult.status === 'fulfilled') setKnowledgeBases(kbRows);
      else failures.push('知识库列表');
      if (failures.length) {
        notify.error(`部分知识数据加载失败：${failures.join('、')}。已保留其他可用数据。`);
      }
      if (documentsResult.status === 'rejected') return;
      const scopedDocRows =
    ...
    try {
      const [rows] = await Promise.all([
        api.get<KnowledgeBucketRead[]>(
          `/api/enterprise/knowledge/documents/${document.id}/buckets?tenant_id=${getRequestTenantId()}${effectiveAgentId ? `&agent_id=${encodeURIComponent(effectiveAgentId)}` : ''}`,
        ),
  ...
  async function loadOkfConcepts(knowledgeBaseId?: string, showLoading = true) {
    ...
    try {
      const rows = await api.get<KnowledgeConceptRead[]>(
        `/api/enterprise/knowledge-bases/${knowledgeBaseId}/okf/concepts?tenant_id=${getRequestTenantId()}${suffix}`,
      );
      setOkfConcepts(rows);
      setOkfLintIssues([]);
    } catch (error) {
  ...
  async function openImportKnowledgeBases(mode: 'plaza' | 'employee' = 'plaza', selectedResourceId?: string) {
    try {
      const agentRows = agents.length ? agents : await api.get<AgentProfileRead[]>(`/api/enterprise/agents?tenant_id=${getRequestTenantId()}`);
      setAgents(agentRows);
      setImportMode(mode);
  ...
  async function loadImportSourceKnowledgeBases(sourceAgentId: string): Promise<KnowledgeBaseRead[]> {
    ...
    try {
      const rows = await api.get<KnowledgeBaseRead[]>(
        `/api/enterprise/knowledge-bases?tenant_id=${getRequestTenantId()}&agent_id=${encodeURIComponent(sourceAgentId)}`,
      );
  ...
  async function exportOkfBundle(targetKnowledgeBase = selectedKnowledgeBase) {
    ...
    try {
      const blob = await api.blob(
        `/api/enterprise/knowledge-bases/${targetKnowledgeBase.id}/okf/export?tenant_id=${getRequestTenantId()}${suffix}`,
      );
  ...
  async function lintOkfBundle(targetKnowledgeBase = selectedKnowledgeBase) {
    ...
    try {
      const result = await api.post<{ status: string; issue_count: number; issues: OkfLintIssue[] }>(
        `/api/enterprise/knowledge-bases/${targetKnowledgeBase.id}/okf/lint?tenant_id=${getRequestTenantId()}${suffix}`,
      );
  ...
  function openKnowledgeGovernance(row: KnowledgeBaseRead) {
    setGoverningKnowledgeBase(row);
    setGovernanceDraft({
      responsibleOrgUnitId: row.responsible_org_unit_id || '',
      accessScope: row.access_scope || 'owner',
      downloadPolicy: row.download_policy || 'restricted',
      organizationAccess: (row.organization_access || []).map((item) => ({
        orgUnitId: item.org_unit_id,
        includeDescendants: item.include_descendants,
      })),
    });
    setGovernanceSelectedUnit(null);
    setGovernanceOrgNames({});
  }

  function selectGovernanceOrganization(unit: OrganizationUnit) {
    setGovernanceSelectedUnit(unit);
    setGovernanceOrgNames((current) => ({ ...current, [unit.id]: unit.name }));
  }

  function addGovernanceAccessRoot() {
    if (!governanceSelectedUnit) return;
    setGovernanceDraft((current) => ({
      ...current,
      accessScope: 'organization',
      organizationAccess: current.organizationAccess.some(
        (item) => item.orgUnitId === governanceSelectedUnit.id,
      )
        ? current.organizationAccess
        : [
            ...current.organizationAccess,
            { orgUnitId: governanceSelectedUnit.id, includeDescendants: true },
          ],
    }));
  }

  async function saveKnowledgeGovernance() {
    if (!governingKnowledgeBase) return;
    if (governanceDraft.accessScope === 'organization' && !governanceDraft.organizationAccess.length) {
      notify.error('组织范围至少需要一个访问组织');
      return;
    }
    setGovernanceSaving(true);
    try {
      const next = await api.put<KnowledgeBaseRead>(
        `/api/enterprise/knowledge-bases/${governingKnowledgeBase.id}/governance`,
        {
          tenant_id: getRequestTenantId(),
          expected_revision: governingKnowledgeBase.revision,
          responsible_org_unit_id: governanceDraft.responsibleOrgUnitId || null,
          access_scope: governanceDraft.accessScope,
          download_policy: governanceDraft.downloadPolicy,
          organization_access: governanceDraft.accessScope === 'organization'
            ? governanceDraft.organizationAccess.map((item) => ({
                org_unit_id: item.orgUnitId,
      ...
      setGoverningKnowledgeBase(null);
      notify.success('已保存知识访问治理');
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) {
        notify.error('知识库已被其他管理员更新，请刷新后重试');
      } else {
        notify.error(error instanceof Error ? error.message : '保存知识访问治理失败');
      }
    } finally {
  ...
  async function runDeleteKnowledgeBase() {
    ...
    const suffix = effectiveAgentId ? `&agent_id=${encodeURIComponent(effectiveAgentId)}` : '';
    try {
      await api.delete(`/api/enterprise/knowledge-bases/${row.id}?tenant_id=${getRequestTenantId()}${suffix}`);
  ...
  async function openKnowledgeBaseVersions(row: KnowledgeBaseRead) {
    ...
    try {
      const versions = await api.get<KnowledgeBaseVersionRead[]>(
        `/api/enterprise/knowledge-bases/${row.id}/versions?tenant_id=${getRequestTenantId()}${suffix}`,
      );
  ...
  async function syncKnowledgeBaseFromOverall(row: KnowledgeBaseRead) {
    ...
    try {
      await api.post(`/api/enterprise/knowledge-bases/${row.id}/sync-from-overall?tenant_id=${getRequestTenantId()}&agent_id=${encodeURIComponent(agentId)}`);
  ...
  async function promoteKnowledgeBaseToOverall(row: KnowledgeBaseRead) {
    ...
    try {
      await api.post(`/api/enterprise/knowledge-bases/${row.id}/promote-to-overall?tenant_id=${getRequestTenantId()}&agent_id=${encodeURIComponent(agentId)}`);
  ...
  async function openBucketEditor(row: KnowledgeBucketRead) {
    ...
    try {
      const chunks = await api.get<KnowledgeChunkRead[]>(
        `/api/enterprise/knowledge/buckets/${row.id}/chunks?tenant_id=${getRequestTenantId()}${effectiveAgentId ? `&agent_id=${encodeURIComponent(effectiveAgentId)}` : ''}`,
      );
  ...
  function renderKnowledgeBaseActions(item: KnowledgeBaseRead) {
    const canReadContent = item.content_access_allowed !== false;
    return (
      <DropdownMenu>
        <DropdownMenuTrigger
          aria-label="知识库操作"
        ...
        >
          ...
          {canGovernKnowledge && (
            <DropdownMenuItem className={MENU_ITEM_CLASS} onSelect={() => openKnowledgeGovernance(item)}>
              <TeamOutlined />
              访问治理
            </DropdownMenuItem>
          )}
          <DropdownMenuItem
            ...
            disabled={!canReadContent}
            onSelect={() => void openKnowledgeBaseVersions(item)}
          >
            <HistoryOutlined />
            版本管理
          </DropdownMenuItem>
          <DropdownMenuItem
            className={MENU_ITEM_CLASS}
            disabled={!canReadContent || item.download_policy !== 'allowed'}
            onSelect={() => void exportOkfBundle(item)}
          >
            <DownloadOutlined />
            导出知识库备份包
          </DropdownMenuItem>
          <DropdownMenuItem
            className={MENU_ITEM_CLASS}
            disabled={okfLoading || !canReadContent}
            onSelect={() => void lintOkfBundle(item)}
          >
            <AuditOutlined />
            知识图谱检查
          </DropdownMenuItem>
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-wrap items-center gap-[6px]">
          ...
          <KTag>
            {row.content_access_allowed === false ? '仅可治理' : '可读取正文'}
          </KTag>
        </div>
      ),
    },
  ...
  const renderMobileKnowledgeBaseCard = (item: KnowledgeBaseRead) => (
    <article
      className={cn(
        ...
        selectedKnowledgeBase?.id === item.id &&
          'border-[var(--gg-cobalt)] ring-2 ring-[color-mix(in_srgb,var(--gg-cobalt)_18%,transparent)]',
      )}
      key={item.id}
    ...
    >
      ...
      <div className="mt-[10px] flex flex-wrap items-center gap-[6px]">
        ...
        <KTag>
          {item.content_access_allowed === false ? '仅可治理' : '可读取正文'}
        </KTag>
  ...
  return (
    <PageShell template="management" aria-busy={loading}>
      ...
      <div className="mt-[20px] mb-[16px] flex flex-wrap items-center justify-end gap-[12px]">
        {!isOverallAgent && canViewOverallKnowledge && (
          <UIButton
            variant="outline"
            onClick={() => {
              clearKnowledgeViewState();
              applyResolvedAgentScope('');
            }}
            ...
            <div className="flex flex-wrap items-center gap-[10px]">
              ...
              <ModelConfigDropdown
                ...
                onChange={(modelId) => {
                  ...
                  window.localStorage.setItem(`${KNOWLEDGE_SEARCH_MODEL_STORAGE_KEY}:${getRequestTenantId()}`, modelId);
                }}
      ...
      <ResourceImportDialog
        ...
        icon={<PlazaResourceIcon kind="knowledge" size="compact" />}
      ...
      <KDialog
        open={Boolean(governingKnowledgeBase)}
        title={governingKnowledgeBase ? `访问治理：${governingKnowledgeBase.name}` : '访问治理'}
        width={920}
        onClose={() => setGoverningKnowledgeBase(null)}
        footer={(
          <>
            <KDialogCancelButton onClick={() => setGoverningKnowledgeBase(null)} />
            <KDialogPrimaryButton
              disabled={governanceSaving}
              onClick={() => void saveKnowledgeGovernance()}
            >
              {governanceSaving ? '保存中…' : '保存治理范围'}
            </KDialogPrimaryButton>
          </>
        )}
      >
        <div className="grid gap-[18px] lg:grid-cols-[360px_minmax(0,1fr)]">
          <section className="rounded-[14px] border border-[#e5e9f2] bg-[#fafbfe] p-[12px]">
            ...
            <OrganizationTreeNavigator
              ...
              selectedId={governanceSelectedUnit?.id || ''}
              selectRootOnInitialize={false}
              onSelect={selectGovernanceOrganization}
            />
          </section>
          <section className="grid content-start gap-[14px]">
            <label className="grid gap-[6px]">
              ...
              <UISelect
                ...
                onValueChange={(value) => setGovernanceDraft((current) => ({
                  ...current,
                  accessScope: value as KnowledgeGovernanceDraft['accessScope'],
                  organizationAccess: value === 'organization' ? current.organizationAccess : [],
                }))}
              >
                <SelectTrigger aria-label="知识访问范围" className={SELECT_TRIGGER_CLASS}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="owner">仅所有者</SelectItem>
            ...
            <label className="grid gap-[6px]">
              ...
              <UISelect
                ...
                onValueChange={(value) => setGovernanceDraft((current) => ({
                  ...current,
                  downloadPolicy: value as KnowledgeGovernanceDraft['downloadPolicy'],
                }))}
              >
                <SelectTrigger aria-label="知识下载策略" className={SELECT_TRIGGER_CLASS}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="restricted">限制下载</SelectItem>
                  <SelectItem value="allowed">允许下载</SelectItem>
                </SelectContent>
              </UISelect>
            </label>
            <div className="grid gap-[7px]">
              ...
              <div className="flex flex-wrap items-center gap-[8px]">
                ...
                <UIButton
                  ...
                  onClick={() => setGovernanceDraft((current) => ({
                    ...current,
                    responsibleOrgUnitId: governanceSelectedUnit?.id || '',
                  }))}
                  type="button"
                  variant="outline"
                >
            ...
            {governanceDraft.accessScope === 'organization' ? (
              <div className="grid gap-[8px]">
                ...
                {governanceDraft.organizationAccess.length ? (
                  governanceDraft.organizationAccess.map((item) => (
                    <div
                    ...
                    >
                      ...
                      <label className="flex items-center gap-[5px] gg-type-caption text-[#596174]">
                        <input
                          ...
                          onChange={(event) => setGovernanceDraft((current) => ({
                            ...current,
                            organizationAccess: current.organizationAccess.map((candidate) => (
                              candidate.orgUnitId === item.orgUnitId
                                ? { ...candidate, includeDescendants: event.target.checked }
                                : candidate
                            )),
                          }))}
                          type="checkbox"
                      ...
                      <UIButton
                        aria-label={`移除访问组织${item.orgUnitId}`}
                        onClick={() => setGovernanceDraft((current) => ({
                          ...current,
                          organizationAccess: current.organizationAccess.filter(
                            (candidate) => candidate.orgUnitId !== item.orgUnitId,
                          ),
                        }))}
                        size="sm"
                        type="button"
...
export function KnowledgeAddPage({ currentUser }: KnowledgePageProps = {}) {
  ...
  const discoveryActionIdsRef = useRef(new Set<string>());
  const [processingDiscoveryIds, setProcessingDiscoveryIds] = useState<Set<string>>(
    () => new Set(),
  );
      ...
      activeJobs.forEach((job) => {
        void api
          .get<KnowledgeIngestJobRead>(
            `/api/enterprise/knowledge/jobs/${job.id}?tenant_id=${getRequestTenantId()}${agentId ? `&agent_id=${encodeURIComponent(agentId)}` : ''}`,
          )
          .then((next) => {
  ...
  async function refreshKnowledgeBases() {
    ...
    try {
      ...
      const rows = await api.get<KnowledgeBaseRead[]>(`/api/enterprise/knowledge-bases?tenant_id=${getRequestTenantId()}${suffix}`);
  ...
  async function loadRecentJobs() {
    ...
    try {
      ...
      const rows = await api.get<KnowledgeIngestJobRead[]>(
        `/api/enterprise/knowledge/jobs?tenant_id=${getRequestTenantId()}${suffix}&limit=8`,
      );
  ...
  async function cancelJob(job: KnowledgeIngestJobRead) {
    ...
    try {
      const next = await api.post<KnowledgeIngestJobRead>(
        `/api/enterprise/knowledge/jobs/${job.id}/cancel?tenant_id=${getRequestTenantId()}`,
      );
      setJobs((prev) => ({ ...prev, [next.id]: next }));
  ...
  async function loadDiscoveriesForJob(job: KnowledgeIngestJobRead) {
    ...
    try {
      const suffix = agentId ? `&agent_id=${encodeURIComponent(agentId)}` : '';
      const rows = await api.get<KnowledgeDiscoveryRead[]>(`/api/enterprise/knowledge/discoveries?tenant_id=${getRequestTenantId()}${suffix}`);
  ...
  async function confirmDiscovery(item: KnowledgeDiscoveryRead) {
    if (!beginDiscoveryAction(item.id)) return;
    try {
      await api.post(`/api/enterprise/knowledge/discoveries/${item.id}/confirm?tenant_id=${getRequestTenantId()}`);
  ...
  async function rejectDiscovery(item: KnowledgeDiscoveryRead) {
    if (!beginDiscoveryAction(item.id)) return;
    try {
      await api.post(`/api/enterprise/knowledge/discoveries/${item.id}/reject?tenant_id=${getRequestTenantId()}`);
  ...
  function beginDiscoveryAction(itemId: string): boolean {
    if (discoveryActionIdsRef.current.has(itemId)) return false;
    discoveryActionIdsRef.current.add(itemId);
    setProcessingDiscoveryIds((current) => new Set(current).add(itemId));
    return true;
  }

  function endDiscoveryAction(itemId: string) {
    discoveryActionIdsRef.current.delete(itemId);
    setProcessingDiscoveryIds((current) => {
    ...
    <div className="knowledge-page knowledge-add-page knowledge-floating-subpage">
      ...
      >
        <DiscoveryColumn
          ...
          processingIds={processingDiscoveryIds}
