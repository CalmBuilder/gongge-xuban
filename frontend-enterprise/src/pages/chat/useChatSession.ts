...
export function useChatSession(options: UseChatSessionOptions = {}) {
  ...
  const tenantId = auth?.user.tenant_id || getRequestTenantId();
  ...
  useEffect(() => {
    ...
    window.addEventListener('gongge-enterprise-agent-scope-refresh', onAgentRefresh);
    return () => window.removeEventListener('gongge-enterprise-agent-scope-refresh', onAgentRefresh);
  }, [loadAgents]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(
      PRODUCT_EVENTS.quickStartCompleted,
      onQuickStartCompleted,
    );
  }, []);
  ...
  useEffect(() => {
    ...
    setModelSetupOpen(true);
  }, [auth, guidesCompleted, modelConfigsLoadError, modelConfigsLoading, selectedModelConfig]);
