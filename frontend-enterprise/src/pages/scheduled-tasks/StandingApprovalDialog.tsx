        ...
        <DialogContent className="max-h-[calc(100dvh-3rem)] gap-0 overflow-y-auto rounded-[18px] p-0 sm:max-w-[720px]" aria-describedby="standing-approval-description">
          <header className="border-b border-[#e8ebf2] bg-[linear-gradient(135deg,#f7f9ff_0%,#ffffff_68%)] px-[24px] py-[20px]">
            <div className="flex items-center gap-[10px]">
              ...
              <div>
                <DialogTitle className="text-[17px] font-semibold text-[#18181a]">长期批准</DialogTitle>
                <DialogDescription id="standing-approval-description" className="mt-[3px] text-[12px] text-[#687087]">
          ...
          <div className="grid gap-[20px] px-[24px] py-[20px]">
            <section aria-label="授权边界" className="overflow-hidden rounded-[14px] border border-[#dfe5f3] bg-white">
              ...
              <div className="border-t border-[#edf0f5] px-[14px] py-[12px]">
                <div className="flex gap-[10px]">
                  <span className="pt-[8px] font-mono text-[10px] text-[#8c96ad]">02</span>
                  <div className="min-w-0 flex-1">
                    <Label htmlFor="standing-target" className="text-[11px] font-medium text-[#687087]">企业微信目标</Label>
                    <Select value={targetId} onValueChange={setTargetId} disabled={loading || candidates.length === 0}>
                      <SelectTrigger id="standing-target" className="mt-[5px] h-[36px] rounded-[9px] border-[#dfe4ee] text-[12px]">
                    ...
                    {!loading && candidates.length === 0 && (
                      <p className="mt-[6px] text-[11px] leading-[1.5] text-[#a35c00]">暂无可用会话。请先让该数字员工接收一条企业微信消息，并在外部连接中开启受控发送。</p>
                    )}
                  </div>
                </div>
              </div>
              <div className="border-t border-[#edf0f5] px-[14px] py-[12px]">
                <div className="flex gap-[10px]">
                  <span className="pt-[3px] font-mono text-[10px] text-[#8c96ad]">03</span>
                  <div className="min-w-0 flex-1">
                    <Label htmlFor="standing-content" className="text-[11px] font-medium text-[#687087]">允许发送的精确正文</Label>
                    <Textarea id="standing-content" value={exactContent} onChange={(event) => setExactContent(event.target.value)} maxLength={4000} className="mt-[5px] min-h-[88px] resize-y rounded-[9px] border-[#dfe4ee] text-[12px] leading-[1.6]" />
                    <p className="mt-[5px] text-right font-mono text-[10px] text-[#9299aa]">{exactContent.length}/4000</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <div className="grid gap-[12px] sm:grid-cols-[180px_1fr] sm:items-start">
              <div>
                <Label htmlFor="standing-validity" className="text-[11px] text-[#687087]">有效期</Label>
                <Select value={validDays} onValueChange={setValidDays}>
                  <SelectTrigger id="standing-validity" className="mt-[5px] h-[36px] rounded-[9px] border-[#dfe4ee] text-[12px]"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <label className="flex cursor-pointer items-start gap-[9px] rounded-[11px] border border-[#f1d7ae] bg-[#fff9ef] px-[12px] py-[10px] text-[11px] leading-[1.55] text-[#825018]">
            ...
            <section aria-label="批准记录" className="border-t border-[#edf0f5] pt-[16px]">
              <div className="mb-[10px] flex items-center justify-between">
                <h3 className="text-[13px] font-semibold text-[#252936]">批准记录</h3>
                <span className="text-[10px] text-[#8b93a6]">只可撤销，不可原地扩权</span>
              </div>
              {revokeTarget && (
                <div role="alert" className="mb-[10px] rounded-[11px] border border-[#f1d7ae] bg-[#fff9ef] px-[12px] py-[10px]">
                  <p className="text-[12px] font-medium text-[#6f4213]">撤销这条长期批准？</p>
                  <p className="mt-[3px] text-[11px] leading-[1.5] text-[#825018]">后续任务若仍需发送，将回到一次性人工审批。</p>
              ...
              <div className="grid gap-[8px]">
                {rules.length === 0 ? <p className="rounded-[11px] bg-[#f7f8fb] px-[12px] py-[14px] text-center text-[11px] text-[#858b9c]">尚未创建长期批准</p> : rules.map((rule) => (
                  <div key={rule.id} className="flex items-start gap-[10px] rounded-[11px] border border-[#e7eaf1] px-[12px] py-[10px]">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[12px] font-medium text-[#252936]">{rule.argument_constraints.content?.equals || '受限正文'}</p>
                      <p className="mt-[3px] flex flex-wrap items-center gap-x-[8px] text-[10px] text-[#838b9d]"><span className="inline-flex items-center gap-[3px]"><Clock3 className="size-[10px]" />至 {formatDateTime(rule.valid_to)}</span><span>{rule.status === 'active' ? '生效中' : '已撤销'}</span></p>
                    </div>
                    {rule.status === 'active' && <Button variant="ghost" size="sm" className="h-[28px] text-[11px] text-[#c43838] hover:text-[#a92020]" onClick={() => setRevokeTarget(rule)}>撤销</Button>}
                  </div>
                ))}
              </div>
            </section>
...
function BoundaryRow({ label, value, index }: { label: string; value: string; index: string }) {
  return (
    <div className="flex gap-[10px] px-[14px] py-[12px]">
      <span className="font-mono text-[10px] text-[#8c96ad]">{index}</span>
      <div className="min-w-0"><p className="text-[11px] font-medium text-[#687087]">{label}</p><p className="mt-[3px] truncate text-[12px] font-semibold text-[#252936]">{value}</p></div>
    </div>
  );
}
