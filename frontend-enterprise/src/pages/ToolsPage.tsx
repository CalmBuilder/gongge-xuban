 ...
 * @Author     : zhanglp8181
 * @File       : ToolsPage.tsx
 * @CallChain  : 企业管理端路由 → ToolsPage → Tools API
 * @Description: 管理工具定义，并按权限配置展示调用人代理或已发布 SOP 流程委托。
 */
...
export default function ToolsPage({ currentUser, onLogout }: ToolPageProps = {}) {
  ...
  const load = () => {
    ...
    return Promise.all([
      ...
      api
        .get<MCPServerRead[]>(`/api/enterprise/mcp-servers?tenant_id=${getRequestTenantId()}`)
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      await api.delete(`/api/enterprise/tools/${row.id}?tenant_id=${getRequestTenantId()}${agentSuffix}`);
  ...
  async function confirmDeleteServer() {
    ...
    try {
      await api.delete(
        `/api/enterprise/mcp-servers/${row.id}?tenant_id=${getRequestTenantId()}${agentQuery}&remove_tools=true`,
      );
      notify.success('已删除');
  ...
  async function openImportTools(mode: 'plaza' | 'employee' = 'plaza', selectedResourceId?: string) {
    try {
      const agentRows = agents.length
        ? agents
        : await api.get<AgentProfileRead[]>(`/api/enterprise/agents?tenant_id=${getRequestTenantId()}`);
      setAgents(agentRows);
      setImportMode(mode);
  ...
  async function loadImportSourceTools(sourceAgentId: string): Promise<ToolRead[]> {
    ...
    try {
      const sourceRows = await api.get<ToolRead[]>(
        `/api/enterprise/tools?tenant_id=${getRequestTenantId()}&agent_id=${encodeURIComponent(sourceAgentId)}`,
      );
  ...
  async function submitImportTools() {
    ...
    try {
      ...
      if (targetAgentId !== agentId) {
        ...
        window.dispatchEvent(new CustomEvent('gongge-enterprise-agent-scope-change', { detail: { agentId: targetAgentId } }));
  ...
  return (
    <div className="min-h-full box-border px-[48px] pt-[32px] pb-[43px] max-[900px]:px-[16px]">
      ...
      <ResourceImportDialog
        ...
        icon={<PlazaResourceIcon kind="tools" size="compact" />}
...
function ToolEditorPage({ mode, currentUser, onLogout }: { mode: 'new' | 'edit' } & ToolPageProps) {
  ...
  const [permissionDefinitions, setPermissionDefinitions] = useState<PermissionDefinitionRead[]>([]);
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    void api
      .get<PermissionDefinitionRead[]>(`/api/organization/permission-definitions?tenant_id=${getRequestTenantId()}&status=active`)
      .then(setPermissionDefinitions)
      .catch(() => setPermissionDefinitions([]));
  }, []);
  ...
  async function save() {
    ...
    const payload = buildToolPayload(
      values,
      isEdit ? tool?.credential_state?.configured_fields || [] : [],
    );
    if (!payload) return;
    setLoading(true);
    try {
      ...
      <div className="grid grid-cols-1 items-start gap-[20px] xl:grid-cols-2">
        <SectionCard title="工具定义" loading={loading && isEdit && !tool}>
          ...
          <ToolFormFields
            ...
            canManagePermission={isEnterpriseAdmin(currentUser)}
            lockName={isEdit}
          />
        </SectionCard>
...
function McpServerEditorPage({ mode, currentUser, onLogout }: { mode: 'new' | 'edit' } & ToolPageProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<MCPServerRead>(`/api/enterprise/mcp-servers/${serverId}?tenant_id=${getRequestTenantId()}`)
      .then((row) => {
        setServer(row);
    ...
    try {
      ...
      try {
        const refreshed = await api.get<MCPServerRead>(
          `/api/enterprise/mcp-servers/${server.id}?tenant_id=${getRequestTenantId()}`,
        );
        setServer(refreshed);
      } catch {
...
function ToolFormFields({
  ...
  permissionDefinitions: PermissionDefinitionRead[];
  canManagePermission: boolean;
  lockName?: boolean;
}) {
  const selectedPermission = permissionDefinitions.find(
    (item) => item.permission_code === values.required_permission_code,
  );
  const isProtected = Boolean(values.required_permission_code);
  ...
  return (
    <div className="flex flex-col gap-[16px]">
      ...
      <div
        className={cn(
          'rounded-[12px] border px-[14px] py-[13px] transition-colors',
          isProtected ? 'border-[#b9c9f6] bg-[#f5f7ff]' : 'border-[#eceef1] bg-[#fafbfc]',
        )}
      >
        <div className="mb-[10px] flex flex-wrap items-start justify-between gap-[8px]">
          <div className="flex min-w-0 flex-col gap-[2px]">
            <span className={FIELD_LABEL_CLASS}>执行权限边界</span>
            <span className={HINT_CLASS}>
              仅对有副作用的工具设置；普通查询工具可保持不受保护。
            </span>
          </div>
          <StatusBadge tone={isProtected ? 'blue' : 'gray'}>
            {isProtected ? '受控执行' : '未设置'}
          </StatusBadge>
        </div>
        <UISelect
          value={values.required_permission_code || '__none__'}
          onValueChange={(value) => setField('required_permission_code', value === '__none__' ? '' : value)}
          disabled={!canManagePermission}
        >
          <SelectTrigger className={cn(SELECT_TRIGGER_CLASS, 'w-full bg-white')} aria-label="执行权限边界">
            <SelectValue placeholder="选择权限点" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="__none__">不设置权限边界</SelectItem>
            {permissionDefinitions.map((item) => (
              <SelectItem key={item.id} value={item.permission_code}>
                {item.name}（{item.permission_code}）
              </SelectItem>
            ))}
          </SelectContent>
        </UISelect>
        ...
        {isProtected && (
          <div className="mt-[10px] border-t border-[#dfe5f6] pt-[10px]">
            <Field
              label="授权来源"
              hint={
                values.permission_authorization_mode === 'workflow_delegated'
                  ? '流程委托：由已发布 SOP 的确定性规则与前置节点授权，申请人无需拥有执行权限。'
                  : '调用人代理：数字员工不能超过当前调用人已有的业务权限。'
              }
            >
              <UISelect
                value={values.permission_authorization_mode || 'caller_and_agent'}
                onValueChange={(value) => setField(
                  'permission_authorization_mode',
                  value as 'caller_and_agent' | 'workflow_delegated',
                )}
                disabled={!canManagePermission}
              >
                <SelectTrigger className={cn(SELECT_TRIGGER_CLASS, 'w-full bg-white')} aria-label="授权来源">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="caller_and_agent">调用人代理</SelectItem>
                  <SelectItem value="workflow_delegated">已发布 SOP 流程委托</SelectItem>
                </SelectContent>
              </UISelect>
            </Field>
          </div>
...
async function loadBucketOptions() {
  const rows = await api.get<ToolRead[]>(`/api/enterprise/tools?tenant_id=${getRequestTenantId()}${currentAgentQuery()}`);
...
function buildToolPayload(
  values: ToolFormValues,
  configuredCredentialFields: Array<'headers' | 'auth' | 'mcp_config'> = [],
) {
  try {
    const headers = parseJson<Record<string, unknown>>(values.headers, {});
    const auth = parseJson<Record<string, unknown>>(values.auth, {});
    const mcpConfig = values.tool_type === 'mcp'
      ? parseJson<Record<string, unknown>>(values.mcp_config, {})
      : {};
    const replacements = { headers, auth, mcp_config: mcpConfig };
    const preserveCredentialFields = configuredCredentialFields.filter(
      (field) => Object.keys(replacements[field]).length === 0,
    );
    return {
      tenant_id: getRequestTenantId(),
      name: String(values.name || '').trim(),
      ...
      preserve_credential_fields: preserveCredentialFields,
