...
export type PaginatorProps = {
  ...
  siblingCount?: number;
  /** Zero-pad page numbers (01, 02, …) to match the 共格 design. Defaults to true. */
  padZero?: boolean;
  className?: string;
  'aria-label'?: string;
};
...
const PILL_CLASS =
  'flex h-[20px] min-w-[20px] items-center justify-center rounded-[6px] px-[12px] gg-type-caption transition-colors';
const ARROW_CLASS =
...
/**
 * Global paginator (共格 design: node 281:2020).
 * Built on the shadcn `Pagination` primitives (nav / ul / li) for structure & a11y,
 * with the 共格 look: 20px pills, 10px zero-padded numbers, `#f6f6f6` active fill,
        ...
        {range.map((item, index) =>
          item === 'ellipsis' ? (
            <PaginationItem key={`ellipsis-${index}`}>
              <span
                aria-hidden="true"
                className="flex h-[20px] items-center justify-center px-[4px] gg-type-caption text-[#999]"
              >
                ···
              </span>
            </PaginationItem>
