...
// Status colors remain semantic while shape and elevation follow the 共格 system.
...
function ToastPill({ variant, message }: { variant: ToastVariant; message: ReactNode }) {
  ...
  return (
    <div
      ...
      aria-live="polite"
      className={cn(
        'pointer-events-auto flex max-w-full items-center gap-[12px] rounded-[var(--gg-radius-control)] border border-solid px-[24px] py-[10px] shadow-[var(--gg-shadow-panel)]',
        container,
      )}
    >
...
/**
 * Global toast helper. `success` / `error` render the 共格 message pill;
