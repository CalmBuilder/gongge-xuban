...
const KnowledgeNavIcon: IconComponent = () => <PlazaResourceArtwork kind="knowledge" size="micro" />;
const GeneralSkillNavIcon: IconComponent = () => <PlazaResourceArtwork kind="general-skills" size="micro" />;
const SopNavIcon: IconComponent = () => <PlazaResourceArtwork kind="skills" size="micro" />;
const ToolNavIcon: IconComponent = () => <PlazaResourceArtwork kind="tools" size="micro" />;

const CAPABILITY_NAV: NavItem[] = [
  { route: EnterpriseRoute.Knowledge, label: '知识库', Icon: KnowledgeNavIcon },
  { route: EnterpriseRoute.GeneralSkills, label: '技能', Icon: GeneralSkillNavIcon },
  { route: EnterpriseRoute.Skills, label: 'SOP', Icon: SopNavIcon },
  { route: EnterpriseRoute.Tools, label: '工具', Icon: ToolNavIcon },
];

const SYSTEM_NAV: NavItem[] = [
  {
    route: EnterpriseRoute.Accounts,
    label: '成员管理',
    Icon: IconAccounts,
    governancePermissions: ['member.read'],
  },
  {
    route: EnterpriseRoute.Organization,
    label: '组织与岗位',
    Icon: IconAccounts,
    governancePermissions: ['organization.read', 'position.read'],
  },
  {
    route: EnterpriseRoute.ReferenceData,
    label: '数据码表',
    Icon: IconAccounts,
    governancePermissions: ['reference_data.read'],
  },
  {
    route: EnterpriseRoute.OrganizationRoles,
    label: '组织角色',
    Icon: IconAccounts,
    governancePermissions: ['authorization.read'],
  },
  {
    route: EnterpriseRoute.ManagementAudit,
    label: '管理审计',
    Icon: IconHistory,
...
function primaryNavItems(
  ...
  const allowedSystemItems = SYSTEM_NAV.filter((item) => (
    item.governancePermissions
...
const SIDEBAR_SHELL_CLASS =
  'gongge-product-sidebar overflow-hidden border-r border-sidebar-border bg-sidebar shadow-[6px_0_18px_-10px_rgba(15,23,42,0.12)] backdrop-blur-[14px] **:data-[slot=sidebar-inner]:bg-sidebar';
...
function CollapsedSidebar({
  ...
  'selected' | 'onNavigate' | 'isAdmin' | 'governancePermissions' | 'sidebarAgent' | 'scopeAgents' | 'selectedAgentId' | 'onSelectAgent' | 'onOpenChat' | 'modelSetupAttention'
> & { onToggle: () => void }) {
  const nameLabel = sidebarAgent
    ...
    : '未选择';
  const primaryItems = primaryNavItems(isAdmin, governancePermissions);
    ...
    <div className="flex h-full w-(--sidebar-width-icon) shrink-0 flex-col items-center gap-[32px] px-[16px] py-[10px]">
      <div className="flex w-full flex-col items-center gap-[10px]">
        <button type="button" title="开放广场" className="flex items-center justify-center p-[10px]">
          <BrandLogo context="management" markOnly />
        </button>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
...
function ManagementSidebar({
  ...
  const primaryItems = useMemo(
    () => primaryNavItems(isAdmin, governancePermissions),
    [governancePermissions, isAdmin],
  );
...
function sessionTitleFor(session: ChatSession, _agent: AgentProfileRead | null): string {
  if (session.title) return normalizeProductDisplayText(session.title);
...
function sessionSubtitleFor(session: ChatSession, _agent: AgentProfileRead | null): string {
  ...
  return recent ? normalizeProductDisplayText(recent) : '新对话';
}
    ...
    <div className="flex h-full w-(--sidebar-width-icon) shrink-0 flex-col items-center gap-[32px] px-[20px] py-[10px]">
      <div className="flex w-full flex-col items-center gap-[10px]">
        <button type="button" title="数字员工广场" onClick={onOpenGallery} className="flex items-center justify-center p-[10px]">
          <BrandLogo context="workspace" markOnly />
        </button>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
