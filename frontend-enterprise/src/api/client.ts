...
export const LOGIN_TENANT_ID = import.meta.env.VITE_TENANT_ID || 'tenant_demo';

export function getRequestTenantId(): string {
  const tenantId = getEnterpriseAuthSession()?.user.tenant_id?.trim();
  if (!tenantId) {
    throw new Error('当前登录会话缺少企业上下文，请重新登录');
  }
  return tenantId;
}
