...
test('S1 GitHub 固定 commit 经供应商边界发现多候选并全部绑定', async ({ page }) => {
  ...
  await expect(dialog.getByText('仅显式调用', { exact: true })).toHaveCount(4);
  await expect(dialog.getByText(/待确认的同包 Skill 引用/)).toHaveCount(4);
  await expect(dialog.getByText('/tdd', { exact: true })).toBeVisible();
  await expect(dialog.getByText('/code-review', { exact: true })).toBeVisible();
  ...
  await dialog.getByRole('button', { name: '固定版本并绑定' }).click();
  expect((await confirmResponse).status()).toBe(200);
  const installedNames = await page.evaluate(async () => {
    ...
    if (!response.ok) throw new Error(`failed to read installed catalog: ${response.status}`);
    const rows = await response.json() as Array<{ name: string }>;
    return rows.map((row) => row.name);
  });
  expect(installedNames).toEqual(expect.arrayContaining(['implement', 'tdd', 'code-review']));
  expect(failures).toEqual([]);
});
...
test('S1 本人私有 GitHub 凭据经加密引用进入后台抓取且完成绑定', async ({ page }) => {
  ...
  expect((await confirmed.json()).installed_revision_ids).toHaveLength(11);
