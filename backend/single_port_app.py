...
from app.brand import site_chat_upstream
from app.main import app


logger = logging.getLogger("gongge_xuban.static")
...
SITE_CHAT_UPSTREAM = site_chat_upstream()
HOP_BY_HOP_HEADERS = {
    "connection",
    "content-length",
    "keep-alive",
...
@app.get("/gongge-xuban-mark.svg", include_in_schema=False)
def brand_icon(request: Request) -> FileResponse:
    name = request.url.path.lstrip("/")
    target = ENTERPRISE_DIST / name
    if not target.exists():
        target = ENTERPRISE_DIST / "gongge-xuban-mark.svg"
