...
def test_rollback_skill_version_restores_content_without_copying_stats() -> None:
    """验证回滚复制旧内容到新发布版本，并保留来源快照而不复用旧版本号。"""
    ...
    with _test_session() as db:
        ...
        snapshots = db.exec(
            select(SkillVersion).where(
                ...
                SkillVersion.skill_id == old_content.skill_id,
            )
        ).all()
    ...
    assert row.content_json["version"] == "1.1.1"
    assert row.content_json["name"] == "旧版购买"
    snapshots_by_version = {snapshot.version: snapshot for snapshot in snapshots}
    assert set(snapshots_by_version) == {"1.0.0", "1.1.0", "1.1.1"}
    assert snapshots_by_version["1.1.1"].derived_from_version_id == snapshots_by_version["1.0.0"].id
    assert snapshots_by_version["1.0.0"].content_json["version"] == "1.0.0"
