...
@Author     : zhanglp8181
@File       : test_context_projection.py
@CallChain  : pytest → context_projection.compact_* → 模型阶段输入
@Description: 验证会话、SOP 与知识证据投影的预算、连续性和原子引用完整性。
"""
...
def test_compact_knowledge_context_keeps_complete_related_group() -> None:
    """同一原子信息的连续切片应合并后进入一条有界知识上下文。"""

    evidence = [
        {
            "chunk_id": f"chunk_{index}",
            "related_group_id": "policy_contact",
            "section_path": "员工手册 / 联系方式",
    ...
    assert "第 6 段内容" in knowledge[0]["content"]


def test_compact_knowledge_context_does_not_split_email_at_character_limit() -> None:
    """投影截断边界落在邮箱内部时必须保留完整邮箱。"""

    prefix = "说明 " + "a" * 790 + " "
    email = "ops@example.test"
    compacted = compact_knowledge_context(
        [{"evidence_pack": [{"content": prefix + email + " 后续说明"}]}]
    )

    content = compacted[0]["retrieved_knowledge"][0]["content"]
    assert email in content
    assert "ops@exam..." not in content
...
def test_step_skill_context_projects_only_canonical_slot_values() -> None:
    """验证模型只看到规范枚举值，不接收会诱发继续仿写的历史同义词。"""
            ...
            "nodes": [
                {
                    "node_id": "collect",
                    "type": "collect_info",
                    "expected_user_info": ["approval_status", "reason"],
                    "metadata": {
                        "value_aliases": {
                            "approval_status": {
                                "已审批": "approved",
    ...
    assert compacted is not None
    assert compacted["current_step"]["slot_value_contract"] == {
        "approval_status": ["approved", "not_approved"]
    }
    assert "已审批" not in str(compacted)
    assert "已通过" not in str(compacted)
    assert "undeclared" not in str(compacted)
