...
@Author     : zhanglp8181
@File       : test_skill_runtime.py
@CallChain  : pytest → SkillRuntime → ChatSession 旧版兼容状态
@Description: 验证旧版多任务路由、任务帧恢复、槽位清理和待输入状态隔离。
"""
...
def test_continue_active_preserves_current_step_against_stale_router_hint():
    """继续同一任务时以会话当前步骤为准，不接受 Router 的滞后步骤提示。"""
    session = ChatSession(
        id="session_test",
        tenant_id="tenant_demo",
        active_skill_id="meeting_room_book",
        active_step_id="confirm_booking",
        slots_json={"date": "2026-07-27", "employee_id": "123456"},
    )
    ...
    assert session.slots_json == {"date": "2026-07-27", "employee_id": "123456"}


def test_continue_active_without_repeated_skill_id_preserves_current_step():
    """Router 省略同一技能 ID 时也不得用建议步骤覆盖已推进的活动步骤。"""
    session = ChatSession(
        id="session_test",
        tenant_id="tenant_demo",
        active_skill_id="meeting_room_book",
...
def test_start_new_task_clears_previous_awaiting_input_and_question():
    """启动新任务时清除旧任务的待输入契约和上一条提问。"""
    session = ChatSession(
        id="session_test",
        tenant_id="tenant_demo",
        active_skill_id="meeting_room_book",
...
def test_switch_to_pending_without_frame_clears_previous_question_context():
    """直接切换待办目标时不得沿用原任务的待输入字段和提问。"""
    session = ChatSession(
        id="session_test",
        tenant_id="tenant_demo",
        active_skill_id="meeting_room_book",
        active_step_id="confirm_booking",
        awaiting_input_json={"expected_fields": ["confirmation"]},
        last_agent_question="请确认预订",
    )
...
def test_restore_frame_without_awaiting_input_drops_stale_expected_fields():
    """恢复未等待输入的任务帧时只保留任务身份，不泄漏旧 expected_fields。"""
    session = ChatSession(
        id="session_test",
        tenant_id="tenant_demo",
        active_skill_id="meeting_room_book",
    ...
    SkillRuntime().restore_task_frame(
        session,
        {
    ...
    assert session.active_skill_id == "purchase"
    assert session.awaiting_input_json == {"task_id": "purchase_task"}
    assert session.last_agent_question is None
