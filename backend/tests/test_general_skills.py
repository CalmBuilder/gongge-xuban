"""
@Time       : 2026/07/29 16:35
@Author     : zhanglp8181
@File       : test_general_skills.py
@CallChain  : pytest → GeneralSkillSelector/AgentLoop → 通用技能与企业知识执行链
@Description: 验证通用技能生命周期、能力选择隔离、知识预检索和聊天运行时集成。
"""
...
from app.agents.branching import ensure_knowledge_base_version, ensure_open_gallery_binding
...
def test_capability_selector_isolates_router_stage_and_receives_knowledge_facts(
    monkeypatch,
) -> None:
    """选择器不得继承同轮 Router 错误理由，但必须接收服务端知识能力事实。"""

    received: dict[str, object] = {}
    monkeypatch.setattr(LLMClient, "__init__", lambda self, model_config: None)

    def fake_generate_json(self, system_prompt, payload):  # noqa: ANN001
        """捕获选择器输入并返回不使用能力的有效结果。"""
    ...
    context = {
        "messages": [
            {"role": "user", "content": "请根据知识库介绍七里坝村"},
        ],
        TURN_STAGE_MESSAGES_KEY: [
            {"role": "user", "content": "Router 请求"},
            {
                "role": "assistant",
                "content": '{"decision":"answer_only","reason":"没有知识库"}',
            },
        ],
    ...
    selector_context = received["conversation_context"]
    assert isinstance(selector_context, dict)
    assert TURN_STAGE_MESSAGES_KEY not in selector_context
    assert received["knowledge_capability"]["knowledge_bases"][0]["name"] == "七里坝村有关情况"


def test_chat_turn_recovers_bound_knowledge_when_selector_fails(monkeypatch) -> None:
    """真实 SQLite 知识链证明 Router 错误理由和 Selector 失败不会再阻断内部资料。"""

    calls: list[str] = []
    selector_payloads: list[dict[str, object]] = []

    def fake_init(self, model_config):  # noqa: ANN001
        """测试模型客户端不建立外部连接。"""

    def fake_generate_json(self, system_prompt, payload):  # noqa: ANN001
        """Router 返回错误理由，Selector 模拟故障以验证词法补偿链。"""
    ...
    def fake_generate_text(self, system_prompt, payload):  # noqa: ANN001
        """确认回复模型获得真实知识证据并生成可核验答案。"""

        calls.append("response")
        retrieved = payload["retrieved_knowledge"]
        assert retrieved
        assert "七里坝村位于陕西省宁强县木古镇" in str(retrieved)
        return "七里坝村位于陕西省宁强县木古镇。"
    ...
    with _test_session() as db:
        ...
        bucket = KnowledgeBucket(
            ...
            bucket_key="qiliba-overview",
            title="七里坝村基本情况",
            summary="七里坝村区位、人口与产业概况",
        )
        chunk = KnowledgeChunk(
            ...
            bucket_id=bucket.id,
            chunk_index=0,
            content="七里坝村位于陕西省宁强县木古镇，村内以农业和乡村旅游为主要产业。",
            summary="七里坝村位于陕西省宁强县木古镇",
            source_ref="七里坝村有关情况.md",
        )
        db.add(document)
        db.add(bucket)
        db.add(chunk)
        ...
        response = AgentLoop(db).handle_turn(
            ChatTurnRequest(
                tenant_id="tenant_demo",
                user_id="user_demo",
                message="请根据当前数字员工绑定的知识库介绍七里坝村，不要联网查询。",
            )
        )

        assert "七里坝村位于陕西省宁强县木古镇" in response.reply
        assert response.step_result is not None
        assert response.step_result.knowledge_results
        assert calls == ["router", "selector", "response"]
        selector_context = selector_payloads[0]["conversation_context"]
        assert isinstance(selector_context, dict)
        assert TURN_STAGE_MESSAGES_KEY not in selector_context
        assert selector_payloads[0]["knowledge_capability"]["available"] is True
        event_types = {
            row.event_type
            for row in db.exec(
        ...
        assert "capability_selection_failed" in event_types
        assert "knowledge_prefetch_finished" in event_types
        assert "knowledge_query_finished" in event_types
