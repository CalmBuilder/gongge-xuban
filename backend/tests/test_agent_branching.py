...
@Author     : zhanglp8181
@File       : test_agent_branching.py
@CallChain  : Agent 资源分支回归 → 知识访问交集 → API 可见性
@Description: 验证 Agent 分支、资源绑定与用户知识范围共同决定资源可见性。
"""
...
def test_seed_publishing_writes_explicit_admin_owner_without_backfilling_user_resources() -> None:
    with _test_session() as db:
        ...
        assert "owner_user_id" not in overall.metadata_json
        assert overall.metadata_json["system_scope"] == "open_gallery_resource_pool"
        assert default_agent.status == "archived"
        assert default_agent.metadata_json["hidden_from_product"] is True
...
def test_copy_overall_scope_to_agent_does_not_auto_bind_open_gallery_knowledge_bases() -> None:
    """复制整体 Agent 时不隐式绑定知识库，且所有者仍可通过整体 Agent 读取。"""
