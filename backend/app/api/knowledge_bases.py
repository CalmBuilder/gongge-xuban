"""
@Time       : 2026/07/29 15:45
@Author     : zhanglp8181
@File       : knowledge_bases.py
@CallChain  : 知识管理 API → 用户/组织/Agent 访问解析 → 知识版本与治理服务
@Description: 提供知识库正文访问、治理元数据、版本、OKF 导出及管理操作接口。
"""
...
from app.knowledge.governance import (
    ...
    active_knowledge_org_access,
    ensure_knowledge_governance_manager,
    update_knowledge_base_governance,
    validate_knowledge_governance_change,
)
from app.knowledge.access import (
    KnowledgeAccessDecision,
    KnowledgeAccessDenied,
    ensure_knowledge_content_access,
...
def list_knowledge_bases(
    ...
    governance_view: bool = Query(False),
    db: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
) -> list[KnowledgeBaseRead]:
    """按正文交集列出知识库；治理视图仅返回操作者可治理的元数据。"""

    ensure_tenant(db, tenant_id)
    if governance_view and agent_id:
        raise HTTPException(status_code=400, detail="Governance view does not accept agent scope")
    ...
    decisions = {
        decision.knowledge_base_id: decision for decision in access.decisions
    }
    if governance_view:
        rows = db.exec(
        ...
        manageable_rows: list[KnowledgeBase] = []
        for row in rows:
            try:
                ensure_knowledge_governance_manager(
                    db,
            ...
            except (KnowledgeGovernanceError, HTTPException):
                continue
            manageable_rows.append(row)
        visible_versions = {
            row.id: ensure_knowledge_base_version(db, row) for row in manageable_rows
        }
        return _knowledge_base_reads(
            db,
            tenant_id=tenant_id,
            rows=manageable_rows,
            versions=visible_versions,
            branch_meta={},
            decisions=decisions,
        )

    allowed_ids = set(access.allowed_knowledge_base_ids)
    ...
    if agent and not agent.is_overall:
        ...
        if not branches:
            return []
        branches = [
            branch for branch in branches if branch.knowledge_base_id in allowed_ids
        ]
        if not branches:
            return []
        ...
        access_by_base = active_knowledge_org_access(
            db,
            tenant_id=tenant_id,
            knowledge_base_ids=list(rows_by_id),
        )
        return [
            knowledge_base_read(
                ...
                organization_access=access_by_base.get(branch.knowledge_base_id),
                access_decision=decisions.get(branch.knowledge_base_id),
            )
            for branch in branches
    ...
    visible_versions = {
        row.id: ensure_knowledge_base_version(db, row) for row in rows
    }
    stats = _knowledge_base_stats(
    ...
    return [
        knowledge_base_read(
            ...
            organization_access=access_by_base.get(row.id),
            access_decision=decisions.get(row.id),
        )
        for row in rows
    ]
...
def create_knowledge_base(
    ...
    """创建所有者默认私有且限制下载的知识库，并建立对应 Agent 或广场绑定。"""
    ...
    append_user_management_audit(
        ...
        current_user=current_user,
        tenant_id=request.tenant_id,
        permission_code="knowledge.manage" if not (agent and not agent.is_overall) else None,
        action="knowledge.create",
        action_kind="create",
        outcome="success",
        resource_type="knowledge_base",
        resource_id=row.id,
        after=_knowledge_governance_audit_snapshot(db, row),
    ...
    return knowledge_base_read(
        ...
        organization_access=[],
        access_decision=_knowledge_access_decision(
            db,
            knowledge_base=row,
            current_user=current_user,
            agent_id=agent_id,
...
def get_knowledge_base(
    ...
    """更新知识内容元数据或 Agent 分支，不改变正式治理范围。"""

    """返回可读知识库，或在显式治理视图下只返回可治理元数据。"""
    ...
    decision = access.decision_for(knowledge_base_id)
    if governance_view:
        ensure_knowledge_governance_manager(
            db,
            knowledge_base=row,
        ...
        visible_version = ensure_knowledge_base_version(db, row)
    else:
        _raise_if_content_denied(decision, knowledge_base_id)
        visible_version = _content_knowledge_version(
            db,
            tenant_id=tenant_id,
            knowledge_base=row,
            agent_id=agent_id,
    ...
    return knowledge_base_read(
        ...
        organization_access=active_knowledge_org_access(
            db,
            tenant_id=tenant_id,
            knowledge_base_ids=[row.id],
        ).get(row.id),
...
@router.put("/{knowledge_base_id}/governance", response_model=KnowledgeBaseRead)
def update_knowledge_governance(
    knowledge_base_id: str,
    request: KnowledgeBaseGovernanceUpdateRequest,
    db: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
) -> KnowledgeBaseRead:
    """更新知识责任和访问治理事实，不改变 Agent 分支内容或产生正文读取授权。"""

    row = _get_knowledge_base(db, request.tenant_id, knowledge_base_id)
    before = _knowledge_governance_audit_snapshot(db, row)
    try:
        ensure_knowledge_governance_manager(
            db,
            knowledge_base=row,
        ...
        change = validate_knowledge_governance_change(
            db,
            tenant_id=request.tenant_id,
            current_user=current_user,
            responsible_org_unit_id=request.responsible_org_unit_id,
            access_scope=request.access_scope,
            download_policy=request.download_policy,
            organization_access=request.organization_access,
        )
        update_knowledge_base_governance(
            db,
            knowledge_base=row,
        ...
        append_user_management_audit(
            ...
            action="knowledge.governance.update",
            action_kind="update",
            outcome="denied",
            resource_type="knowledge_base",
            resource_id=row.id,
            target_org_unit_id=row.responsible_org_unit_id,
            before=before,
            detail={"status_code": error.status_code, "reason": error.detail},
        )
        db.commit()
        raise
    except KnowledgeGovernanceError as error:
        status_code = 409 if str(error) == "KNOWLEDGE_BASE_REVISION_CONFLICT" else 400
        append_user_management_audit(
            db,
            current_user=current_user,
            tenant_id=request.tenant_id,
            ...
            target_org_unit_id=row.responsible_org_unit_id,
            before=before,
            detail={"status_code": status_code, "reason": str(error)},
        )
        db.commit()
        raise HTTPException(
    ...
    return knowledge_base_read(
        row,
        _knowledge_base_stats(db, request.tenant_id, [version.id]).get(row.id, {}),
        version_row=version,
        organization_access=active_knowledge_org_access(
            db,
            tenant_id=request.tenant_id,
...
def list_knowledge_base_versions(
    ...
    """列出正文可访问知识库的版本元数据，并按 Agent 分支隔离私有版本。"""

    row = _get_knowledge_base(db, tenant_id, knowledge_base_id)
    _content_knowledge_version(
        db,
        tenant_id=tenant_id,
        knowledge_base=row,
        agent_id=agent_id,
...
def list_okf_concepts(
    ...
    knowledge_base = _get_knowledge_base(db, tenant_id, knowledge_base_id)
    version = _content_knowledge_version(
        db,
        tenant_id=tenant_id,
...
def export_okf(
    ...
    append_user_management_audit(
        ...
        target_org_unit_id=kb.responsible_org_unit_id,
        detail={"agent_id": agent_id, "version_id": version.id},
    )
    db.commit()
...
def knowledge_base_read(
    ...
    organization_access: list[KnowledgeBaseOrgAccess] | None = None,
    access_decision: KnowledgeAccessDecision | None = None,
) -> KnowledgeBaseRead:
    """将知识库内容版本与正式治理事实合并为管理端契约。"""
    ...
    return KnowledgeBaseRead(
        ...
        organization_access=[
            KnowledgeBaseOrgAccessRead(
                id=access.id,
                org_unit_id=access.org_unit_id,
                include_descendants=access.include_descendants,
                status=access.status,
            )
            for access in (organization_access or [])
        ],
        content_access_allowed=access_decision.allowed if access_decision else True,
        content_access_reason=(
            access_decision.member_scope_reason
            if access_decision and access_decision.allowed
            else (access_decision.denial_reason or "knowledge_access_denied")
            if access_decision
            else "allowed"
        ),
...
def _knowledge_base_reads(
    db: Session,
    *,
    tenant_id: str,
    rows: list[KnowledgeBase],
    versions: dict[str, KnowledgeBaseVersion],
    branch_meta: dict[str, dict[str, str]],
    decisions: dict[str, KnowledgeAccessDecision],
) -> list[KnowledgeBaseRead]:
    """批量组装知识库治理视图，统计和组织访问均只查询一次。"""
    ...
    return [
        knowledge_base_read(
            row,
            stats.get(row.id, {}),
            version_row=versions.get(row.id),
            branch_meta=branch_meta.get(row.id),
            organization_access=access_by_base.get(row.id),
            access_decision=decisions.get(row.id),
        )
...
def _knowledge_access_decision(
    db: Session,
    *,
    ...
    """计算写入响应的当前正文访问结果，避免把治理成功误报为可读正文。"""

    return resolve_knowledge_access(
        db,
        tenant_id=knowledge_base.tenant_id,
        current_user=current_user,
        agent_id=agent_id,
        requested_knowledge_base_ids=[knowledge_base.id],
    ).decision_for(knowledge_base.id)


def _content_knowledge_version(
    db: Session,
    *,
    tenant_id: str,
    knowledge_base: KnowledgeBase,
    ...
    audit_action: str = "knowledge.read",
) -> KnowledgeBaseVersion:
    """确认正文交集后返回 Agent 当前分支或无 Agent 场景的知识库当前版本。"""
    ...
    try:
        ...
        append_user_management_audit(
            ...
            target_org_unit_id=knowledge_base.responsible_org_unit_id,
            detail={
                "agent_id": agent_id,
                "reason": error.reason,
                "download_requested": require_download,
            },
        )
        db.commit()
        raise HTTPException(status_code=404, detail="Knowledge resource not visible") from error
    if agent_id:
        return _visible_knowledge_version(
            db,
            tenant_id,
            knowledge_base.id,
            agent_id,
        )
    return ensure_knowledge_base_version(db, knowledge_base)


def _knowledge_governance_audit_snapshot(
    db: Session,
    knowledge_base: KnowledgeBase,
) -> dict[str, object]:
    """返回不含知识正文的治理字段和规范化组织根快照。"""
    ...
    return {
        ...
        "responsible_org_unit_id": knowledge_base.responsible_org_unit_id,
        "access_scope": knowledge_base.access_scope,
        "download_policy": knowledge_base.download_policy,
        ...
        "organization_access": [
            ...
            for row in organization_access
        ],
    }


def _raise_if_content_denied(
    decision: KnowledgeAccessDecision | None,
    knowledge_base_id: str,
) -> None:
    """把不存在和未命中交集统一映射为 404，避免资源枚举。"""

    if decision is None or not decision.allowed:
        raise HTTPException(
            status_code=404,
            detail=f"Knowledge resource {knowledge_base_id} is not visible",
        )
