...
@Description: 管理工具定义、授权来源、SOP 白名单并提供受控测试执行入口。
"""
...
def tool_read(row: Tool, metadata: dict[str, Any] | None = None) -> ToolRead:
    ...
    return ToolRead(
        ...
        auth={},
        mcp_config={},
        credential_state=_tool_credential_state(row),
...
def create_tool(
    ...
    row = Tool(
        ...
        required_permission_code=request.required_permission_code,
        permission_authorization_mode=request.permission_authorization_mode,
...
def update_tool(
    ...
    """更新工具配置，并在保存前阻断失效权限或未绑定 SOP 的受保护工具。"""
    ...
    row.url = request.url
    _apply_tool_credential_update(
        row,
        request,
        allow_preserve=not (agent and not agent.is_overall and source_tool_id != row.id),
    )
    ...
    _validate_required_permission(db, request)
    row.required_permission_code = request.required_permission_code
...
def test_tool(
    ...
    return ToolExecutor(db).execute(
        ...
        execution_org_unit_id=request.organization_unit_id,
    )


def _validate_required_permission(
    db: Session, request: ToolCreateRequest | ToolUpdateRequest
) -> None:
    """校验工具引用的权限目录事实，并强制受保护工具显式绑定 SOP。"""

    permission_code = (request.required_permission_code or "").strip()
    if not permission_code:
        request.required_permission_code = None
        return
    if not request.allowed_skills:
        raise HTTPException(status_code=422, detail="受保护工具必须至少显式绑定一个 SOP")
    from app.db.models import PermissionDefinition
    ...
    if permission is None:
        raise HTTPException(status_code=422, detail=f"权限点不存在或未启用：{permission_code}")
...
def _tool_credential_state(row: Tool) -> ToolCredentialState:
    """生成不包含值的工具凭据配置摘要。"""
    ...
    return ToolCredentialState(
        configured_fields=[name for name, value in mappings.items() if value],
        header_keys=sorted(str(key) for key in mappings["headers"]),
        auth_keys=sorted(str(key) for key in mappings["auth"]),
        mcp_config_keys=sorted(str(key) for key in mappings["mcp_config"]),
    )


def _apply_tool_credential_update(
    row: Tool,
    request: ToolUpdateRequest,
    *,
    allow_preserve: bool,
) -> None:
    """按显式保留、替换或清除语义更新工具凭据，复制分支禁止继承来源秘密。"""

    preserve = set(request.preserve_credential_fields)
    clear = set(request.clear_credential_fields)
    overlap = preserve & clear
    if overlap:
        raise HTTPException(
            status_code=422,
            detail=f"Credential fields cannot be both preserved and cleared: {sorted(overlap)}",
        )
    updates = {
        "headers": ("headers_json", request.headers),
        "auth": ("auth_json", request.auth),
        "mcp_config": ("config_json", request.mcp_config),
    }
    for field_name, (attribute_name, replacement) in updates.items():
        if field_name in clear:
            setattr(row, attribute_name, {})
        elif field_name in preserve and allow_preserve:
            continue
        else:
            setattr(row, attribute_name, replacement)
