...
def session_read(row: ChatSession, *, is_scheduled: bool = False) -> ChatSessionRead:
    """读取会话创建时锚定的员工版本；历史行明确返回 legacy 而不伪造快照。"""

    is_legacy_agent_session = bool(
        row.agent_id
        and row.agent_profile_revision is None
        and row.capability_snapshot_json is None
        and row.origin is None
    )
    return ChatSessionRead(
        ...
        capability_snapshot=row.capability_snapshot_json,
        origin="legacy" if is_legacy_agent_session else row.origin,
        title=row.title,
...
def create_chat_session(
    ...
    anchor_chat_session(db, row, agent, origin=request.origin)
    db.add(row)
    db.commit()
    db.refresh(row)
...
def _bind_request_to_session_agent(
    ...
    agent = _ensure_chat_agent_available(db, request.tenant_id, request.agent_id, current_user)
    anchor_chat_session(db, chat_session, agent, origin="direct")
