...
@Author     : zhanglp8181
@File       : auth.py
@CallChain  : FastAPI 认证依赖/登录 API → token 校验 → User 租户成员状态
@Description: 签发并校验租户账号 token，在认证边界立即阻止非活动成员继续工作。
"""
...
def verify_password(password: str, stored_hash: str) -> bool:
    try:
        algorithm, salt, _stored_digest = stored_hash.split("$", 2)
    except ValueError:
        return False
...
def ensure_active_member(user: User) -> None:
    """拒绝已停用或离职的租户成员，不删除其历史 actor 记录。"""
