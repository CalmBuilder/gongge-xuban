...
@Description: 选择并执行通用技能，同时隔离独立能力决策并接收可验证的知识能力事实。
"""
...
GENERAL_SKILL_SELECTION_OUTPUT = {
    ...
    "knowledge_mode": "auto | required | disabled",
    "confidence": "number",
    "reason": "string?",
}
...
class GeneralSkillSelector:
    def decide(
        ...
        knowledge_capability = selector_context.pop(
        ...
        if decision.use_knowledge:
            decision = decision.model_copy(update={"knowledge_mode": "required"})
        elif decision.knowledge_mode == "disabled":
            decision = decision.model_copy(
                update={"use_knowledge": False, "knowledge_query": None}
            )
        if (
            isinstance(knowledge_capability, dict)
            and knowledge_capability.get("available") is False
        ):
            decision = decision.model_copy(
                update={
                    "use_knowledge": False,
