...
@Description : 负责知识入库、受限检索、意图感知排序、证据组织和知识发现状态转换。
"""
...
SEARCH_MIN_DESIRED_EVIDENCE_SCORE = 2.0
DESIRED_EVIDENCE_SCORE_WEIGHT = 0.35
DESIRED_EVIDENCE_SCORE_CAP = 4.0
QUERY_TYPE_MATCH_BONUS = 2.0
RELATED_CHUNK_MAX_COUNT = 6
RELATED_CHUNK_MAX_CHARS = 4_800
...
def validate_discovered_skill(payload: dict[str, Any]) -> SkillCard:
    ...
    for index, edge in enumerate(raw_edges if isinstance(raw_edges, list) else []):
...
class KnowledgeService:
    ...
    def search(self, request: KnowledgeSearchRequest, model_config: ModelConfig | None = None) -> KnowledgeSearchResponse:
        ...
        with observed_span(
            ...
            degraded = any(phase.endswith("_lexical_fallback") for phase in phases)
            sufficiency = _assess_evidence_sufficiency(
                response,
                request.desired_evidence,
            )
            if sufficiency.satisfied:
                outcome = "evidence_found"
            elif (
                sufficiency.evidence_count
                or response.selected_documents
                or response.selected_buckets
                or response.selected_concepts
            ):
    ...
    def _search(self, request: KnowledgeSearchRequest, model_config: ModelConfig | None = None) -> KnowledgeSearchResponse:
        ...
        with observed_span(
            ...
            if model_config:
                ...
                route_failed = any(
                    ...
                    for item in route_trace[trace_start:]
                )
                if route_failed:
                    selected_document_ids = [
                        row.id
                        for row in _score_documents(
                            query,
                            documents,
                            desired_evidence=request.desired_evidence,
                        )[:5]
        ...
        with observed_span(
            ...
            if model_config:
                ...
                route_failed = any(
                    str(item.get("phase") or "") in {
                        "bucket_selection_failed",
                        "bucket_selection_invalid",
                    }
                    for item in route_trace[trace_start:]
                )
                if route_failed:
    ...
    def _build_chunks(
        ...
        for bucket in buckets:
            ...
            for section in section_sources:
                ...
                part_groups = _chunk_text_related_groups(
                    content,
                ...
                for group_index, parts in enumerate(part_groups):
                    ...
                    related_rows = related_rows_by_group.setdefault(
                        related_group_id,
                        [],
                    )
                    for part in parts:
                        ...
                        source_path = (
                            f"{document.filename} / "
                            f"{section.get('path') or bucket.title} / "
                            f"evidence {local_index + 1}"
                        )
                        row = KnowledgeChunk(
                            tenant_id=tenant_id,
                            knowledge_base_id=knowledge_base_id,
            ...
            for related_rows in related_rows_by_group.values():
                ...
                for related_index, row in enumerate(related_rows):
                    related_window = _related_chunk_window(
                        related_rows,
                        related_index,
                    )
                    ...
                    row_metadata.update(
                        {
                            ...
                            "related_previous_chunk_id": (
                                related_ids[window_index - 1]
                                if window_index > 0
                                else None
                            ),
                            "related_next_chunk_id": (
                                related_ids[window_index + 1]
    ...
    def _select_documents_with_llm(
        ...
        *,
        ...
        if not isinstance(ids, list):
            trace.append(
                {"phase": "document_route_invalid", "message": "模型未返回文档 ID 数组"}
            )
            return []
        ...
        *,
        ...
        payload = {
            ...
            "buckets": [
                ...
                for bucket in buckets[:SEARCH_BUCKET_ROUTE_LIMIT]
            ],
        }
        try:
        ...
        if not isinstance(ids, list):
            trace.append(
                {"phase": "bucket_selection_invalid", "message": "模型未返回内部索引 ID 数组"}
            )
            return []
    ...
    def _load_chunks_for_buckets(
        ...
        """按来源顺序读取候选片段；检索阶段可加载完整候选后再相关性裁剪。"""
    ...
    def _confirm_tool(self, suggestion: KnowledgeDiscoverySuggestion, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        self.db.add(row)
        self.db.flush()
        self._bind_discovered_resource(suggestion, "tool", row.id, "active")
        self.db.refresh(row)
    ...
    def _confirm_skill(self, suggestion: KnowledgeDiscoverySuggestion, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        self.db.add(row)
        self.db.flush()
        self._bind_discovered_resource(suggestion, "skill", row.id, "inactive")
        self.db.refresh(row)
        return {"status": "created", "skill_id": row.id}

    def _bind_discovered_resource(
        self,
        ...
        elif knowledge_base and is_open_gallery_resource(
            self.db,
            suggestion.tenant_id,
            "knowledge_base",
            knowledge_base,
...
def _looks_like_heading(text: str) -> bool:
    """识别 Markdown、中文章节编号和短标题行。"""

    stripped = text.strip()
    return bool(
        re.match(r"^#{1,6}\s+\S", stripped)
        or re.match(
            r"^第[一二三四五六七八九十百千万0-9]+[章节篇部分]\b",
            stripped,
        )
        or (len(stripped) <= 48 and stripped.endswith(("：", ":")))
    )
    ...
    for row in documents:
        searchable_text = " ".join(
            [
                ...
                row.filename,
                json.dumps(
                    (row.metadata_json or {}).get("document_card", {}),
                    ensure_ascii=False,
                )[:4000],
            ]
        )
        primary_score = _score_text(query, searchable_text)
        if primary_score < SEARCH_MIN_DOCUMENT_SCORE:
            continue
        score = primary_score + _desired_evidence_bonus(
            desired_evidence,
            searchable_text,
        )
...
def _score_buckets(
    ...
    """按主查询筛选知识桶，再结合用途标签和证据要求稳定重排。"""

    scored: list[tuple[float, KnowledgeBucket]] = []
    for row in buckets:
        metadata = row.metadata_json or {}
        metadata_text = json.dumps(metadata, ensure_ascii=False)[:3000]
        searchable_text = " ".join([row.title, row.summary, metadata_text])
        primary_score = _score_weighted_text(
            query,
            (row.title, 3.0),
            (row.summary, 1.8),
            (
                " ".join(str(item) for item in metadata.get("section_paths", [])),
                1.4,
            ),
            (str(metadata.get("content") or ""), 0.6),
        )
        if primary_score < SEARCH_MIN_BUCKET_SCORE:
            continue
        score = primary_score + _desired_evidence_bonus(
            desired_evidence,
            searchable_text,
...
def _rank_chunks(
    ...
    for chunk in chunks:
        ...
        searchable_text = f"{chunk.summary or ''} {chunk.content}"
        primary_score = max(
            _score_text(query, searchable_text),
        ...
        if primary_score < SEARCH_MIN_CHUNK_SCORE:
            continue
        score = (
            primary_score
            + section_bonus
            + _desired_evidence_bonus(desired_evidence, searchable_text)
        )
        scored.append(
            (
                (
                    score,
                    -bucket_rank.get(chunk.bucket_id, 999),
                    -chunk.chunk_index,
                ),
                chunk,
            )
...
def _expand_related_chunks(
    ...
    for chunk in candidates:
        group_id = str(
            (chunk.metadata_json or {}).get("related_group_id") or ""
        ).strip()
        if group_id:
    ...
    for hit in ranked_hits:
        ...
        if not group and group_id:
            ...
            hit_index = next(
                (
                    index
                    for index, chunk in enumerate(full_group)
                    if chunk.id == hit.id
                ),
                0,
            )
...
def _related_chunk_window(
    ...
    """围绕命中片段选择最多六段、总计不超过 4800 字符的连续窗口。"""
    ...
    while len(selected) < RELATED_CHUNK_MAX_COUNT:
        ...
        if (
            not added
            and center_index - distance < 0
            and center_index + distance >= len(chunks)
        ):
            break
        distance += 1
...
def _build_evidence_pack(
    ...
    for chunk in chunks:
        ...
        if (
            score < SEARCH_MIN_EVIDENCE_SCORE
            and not related_group_id
            and not is_direct_hit
        ):
            continue
        evidence_alignment_score = (
            _score_text(desired_evidence, searchable_text) if desired_evidence else None
        )
        evidence.append(
            {
                "chunk_id": chunk.id,
                ...
                "evidence_alignment_score": (
                    round(evidence_alignment_score, 2)
                    if evidence_alignment_score is not None
                    else None
                ),
                "confidence_reason": (
                    "与命中片段属于同一连续原子信息"
...
def _assess_evidence_sufficiency(
    ...
    desired_evidence: str | None,
) -> KnowledgeEvidenceSufficiency:
    """用有界词法分数判断证据是否覆盖期望要求，并输出可持久化解释。"""

    evidence_items = [
        item
        for item in [*response.evidence_pack, *response.okf_citations]
        if isinstance(item, dict)
    ]
    required = bool((desired_evidence or "").strip())
    if not evidence_items:
        return KnowledgeEvidenceSufficiency(
            required=required,
            alignment_threshold=(
                SEARCH_MIN_DESIRED_EVIDENCE_SCORE if required else None
            ),
            reason="no_evidence",
        )
    if not required:
        return KnowledgeEvidenceSufficiency(
            ...
            aligned_evidence_count=len(evidence_items),
            reason="evidence_available",
        )

    aspects = _desired_evidence_aspects(desired_evidence or "")
    score_rows = [
        [
            _evidence_alignment_score(
                item,
                aspect,
                use_explicit=len(aspects) == 1,
            )
    ...
    covered_aspects = [
        ...
        if any(
            scores[index] >= SEARCH_MIN_DESIRED_EVIDENCE_SCORE
    ...
    aligned_count = len(
        [
            scores
            for scores in score_rows
            if any(score >= SEARCH_MIN_DESIRED_EVIDENCE_SCORE for score in scores)
        ]
    )
    max_score = max(
        (score for scores in score_rows for score in scores),
        default=0.0,
    )
    satisfied = bool(aspects) and len(covered_aspects) == len(aspects)
    return KnowledgeEvidenceSufficiency(
        required=True,
        satisfied=satisfied,
        evidence_count=len(evidence_items),
        ...
        max_alignment_score=round(max_score, 2),
        alignment_threshold=SEARCH_MIN_DESIRED_EVIDENCE_SCORE,
        reason=(
            "desired_evidence_aligned"
            if satisfied
            else "desired_evidence_not_aligned"
        ),
    )
...
def _evidence_alignment_score(
    ...
    *,
    ...
    """优先读取证据包已有分数，否则从受限展示字段重新计算对齐度。"""

    if _is_provenance_aspect(desired_evidence):
        return (
            SEARCH_MIN_DESIRED_EVIDENCE_SCORE
            if _evidence_has_provenance(evidence)
            else 0.0
        )
    explicit_score = evidence.get("evidence_alignment_score")
    if use_explicit and isinstance(explicit_score, int | float):
    ...
    return _score_text(desired_evidence, searchable_text)


def _is_provenance_aspect(aspect: str) -> bool:
    """识别只要求证据可追溯性的独立要点，避免把元数据误当正文关键词。"""

    normalized = re.sub(r"\s+", "", aspect).strip("。.!！?？")
...
def _evidence_has_provenance(evidence: dict[str, Any]) -> bool:
    """确认引用至少携带一个可定位到知识实体或原始来源的稳定字段。"""

    return any(
        bool(str(evidence.get(key) or "").strip())
        for key in (
            "source_path",
            "document_id",
            "chunk_id",
...
def _desired_evidence_aspects(desired_evidence: str) -> list[str]:
    """把自然语言证据要求拆成少量必须全部覆盖的独立要点。"""

    raw_aspects = re.split(
        r"(?:、|，|,|；|;|以及|并且|和|及|与|\band\b)",
        desired_evidence,
        flags=re.IGNORECASE,
    )
    aspects: list[str] = []
    for raw_aspect in raw_aspects:
        aspect = re.sub(r"\s+", " ", raw_aspect).strip(" 。.!！?？")
        if len(aspect) >= 2 and aspect not in aspects:
            aspects.append(aspect)
    normalized = re.sub(r"\s+", " ", desired_evidence).strip()
    return aspects[:8] or ([normalized] if normalized else [])
...
def _score_text(query: str, text: str) -> float:
    ...
    if not variants:
        return 0.0
    scores = sorted(
        (_score_single_query(variant, text) for variant in variants),
        reverse=True,
    )
...
def _desired_evidence_bonus(desired_evidence: str | None, text: str) -> float:
    """将证据要求转换为有上限的二级排序加分，不参与主相关性准入。"""

    if not desired_evidence:
        return 0.0
    return min(
        DESIRED_EVIDENCE_SCORE_CAP,
        _score_text(desired_evidence, text) * DESIRED_EVIDENCE_SCORE_WEIGHT,
    )


def _score_weighted_text(query: str, *fields: tuple[str, float]) -> float:
    """按字段重要性汇总查询相关性，避免正文噪声压过标题和摘要。"""

    return sum(
        _score_text(query, text) * weight
