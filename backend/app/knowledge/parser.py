...
@Author     : zhanglp8181
@File       : parser.py
@CallChain  : KnowledgeService.run_ingest_job → extract_text → 文档结构化入库
@Description: 从受支持文档提取文本，并尽量保留 PDF 页码和 DOCX 标题层级。
"""
...
def _extract_pdf(content: bytes) -> str:
    """按页提取 PDF 正文并生成可被章节解析器识别的 Markdown 页标题。"""
    try:
...
def _extract_docx(content: bytes) -> str:
    """提取 DOCX 段落和表格，并把标准中英文标题样式转换为 Markdown。"""
    try:
...
def _docx_heading_level(style_name: str) -> int | None:
    """识别 Word 的 Heading/标题 1—6 样式并返回对应 Markdown 层级。"""
