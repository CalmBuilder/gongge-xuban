    ...
    for row in concepts:
        ...
        for link in row.links_json or []:
            ...
            if target in inbound:
                inbound[target] += 1
            elif not is_supported_knowledge_target(str(link.get("target") or "")):
    ...
    body = "\n".join(
        [
            ...
            *(bucket_lines or ["- 暂无知识桶"]),
            ...
            "# Citations",
            "",
            f"[1] [Original document]({knowledge_uri(f'documents/{document.id}')})",
        ]
    )
    frontmatter = {
        ...
        "resource": knowledge_uri(f"documents/{document.id}"),
...
def _source_section_concept(document: KnowledgeDocument, document_concept_id: str, section: dict[str, Any]) -> dict[str, Any]:
    ...
    frontmatter = {
        ...
        "resource": f"{knowledge_uri(f'documents/{document.id}')}#section={section_id}",
        "tags": ["source-section"],
        "timestamp": document.updated_at.isoformat(),
...
def _bucket_concept(document: KnowledgeDocument, document_concept_id: str, bucket: KnowledgeBucket) -> dict[str, Any]:
    ...
    frontmatter = {
        ...
        "resource": knowledge_uri(f"buckets/{bucket.id}"),
...
KNOWLEDGE_URI_SCHEME = "gongge-xuban"


def knowledge_uri(path: str) -> str:
    return f"{KNOWLEDGE_URI_SCHEME}://knowledge/{path.lstrip('/')}"


def is_supported_knowledge_target(target: str) -> bool:
    prefixes = (
        "http://",
        "https://",
        f"{KNOWLEDGE_URI_SCHEME}://",
    )
