...
@CallChain  : AgentLoop → SkillRuntime → ChatSession 旧版兼容字段
@Description: 管理旧版多任务会话帧、待输入上下文及 Router 决策的防御性状态切换。
"""
...
class SkillRuntime:
    ...
    def _activate_decision_target(
        ...
        """激活旧版 Router 目标，并阻止任务切换串入旧提问或倒退当前步骤。"""
        if selected_frame:
        ...
        preserve_active_step = (
            ...
            and (
                not decision.target_skill_id
                or decision.target_skill_id == session.active_skill_id
            )
        )
...
def _activate_frame(session: ChatSession, frame: dict[str, Any]) -> None:
    """恢复旧版任务帧，并清除帧中未携带的待输入状态以避免跨任务泄漏。"""
    ...
    session.awaiting_input_json = (
        dict(awaiting_input) if isinstance(awaiting_input, dict) else None
    )
