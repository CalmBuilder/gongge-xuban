...
@Description: 编排聊天执行循环，并持久化回复、工具、知识任务、会话状态与审计事件。
"""
...
class AgentLoop:
    ...
    def _protect_deterministic_runtime_route(
        self,
        chat_session: ChatSession,
        router_decision: RouterDecision,
        skills: list[Skill],
    ) -> RouterDecision:
        """活动确定性实例存在时，以持久化游标覆盖模型的提前退出、切换或过期节点。"""

        deterministic_runtime = getattr(self, "deterministic_runtime", None)
        if deterministic_runtime is None:
            return router_decision
        previous_skill_id = chat_session.active_skill_id
        previous_step_id = chat_session.active_step_id
        instance = deterministic_runtime.synchronize_active_instance(chat_session)
        if instance is None:
            return router_decision
        active_skill = next(
            (skill for skill in skills if skill.skill_id == instance.skill_id),
            None,
        )
        if not DeterministicSopCoordinator.is_enabled(active_skill):
            raise ValueError("活动确定性 SOP 实例对应的已发布技能不可用")
        protected_decision = router_decision.model_copy(
            update={
                "decision": "continue_active",
                "selected_task_id": None,
                "target_skill_id": instance.skill_id,
                "target_step_id": instance.current_node_id,
                "reason": "活动确定性 SOP 由持久化实例游标继续执行。",
                "task_frames": [],
                "pending_tasks": [],
                "task_updates": [],
                "created_tasks": [],
        ...
        if (
            ...
            or router_decision.target_skill_id != instance.skill_id
            or router_decision.target_step_id != instance.current_node_id
            or previous_skill_id != instance.skill_id
            or previous_step_id != instance.current_node_id
        ):
            self.events.record(
                chat_session.tenant_id,
                chat_session.id,
                "deterministic_route_protected",
                {
                    "instance_id": instance.id,
                    "original_decision": router_decision.decision,
                    "original_skill_id": previous_skill_id,
                    "original_step_id": previous_step_id,
                    "protected_skill_id": instance.skill_id,
                    "protected_step_id": instance.current_node_id,
                },
            )
    ...
    def _stream_general_skill_response(
        ...
        self.db.commit()
        self.db.refresh(chat_session)
        replacement = self._stream_final_reply_replacement(
            chat_session,
            streamed_reply,
            reply,
            user_message_id,
    ...
    def handle_turn_stream(self, request: ChatTurnRequest) -> Iterator[dict[str, object]]:
        ...
        def finalize_turn_once(
            ...
            """至多持久化一次本轮回复，并返回实际写入的规范化文本。"""

            nonlocal finalized_turn_reply, turn_finalized
            if turn_finalized:
                return finalized_turn_reply or final_reply
        ...
        def stream_failure_response(
            ...
            for chunk in self.response_generator.chunk_text(reply):
                ...
                self._pace_stream()
            streamed_reply = reply
            reply = finalize_turn_once(target_session, reply, step_result, request.message)
            self.db.commit()
    ...
    def _stream_final_reply_replacement(
        self,
        chat_session: ChatSession,
        streamed_reply: str,
        finalized_reply: str,
        ...
        """当持久化规范化改变文本时，发送一次最终替换事件统一客户端内容。"""

        if finalized_reply == streamed_reply:
            return None
        return self._stream_event(
            "stream_replace",
            chat_session,
            self._turn_payload(
                {"content": finalized_reply},
                user_message_id,
            ),
        )
    ...
    def _execute_tool_action_cycle(
        ...
        for iteration in range(max_actions):
            ...
            if DeterministicSopCoordinator.is_enabled(active_skill):
                runtime_plan = self.deterministic_runtime.record_tool_result(
                    chat_session, tool_call, tool_result
                )
            if stream_events is not None:
                stream_events.append(
                    (
            ...
            if runtime_plan is not None:
                step_result = self.deterministic_runtime.merge_plan(step_result, runtime_plan)
                if runtime_plan.action is RuntimeAction.COMPLETE:
                    payload = self._tool_loop_decision_payload(
                        iteration + 1, "deterministic_runtime_completed"
                    )
                    self.events.record(
                        request.tenant_id,
                        chat_session.id,
                ...
                if runtime_plan.action in {
                    ...
                    break
                if (
                    runtime_plan.action is RuntimeAction.QUERY_KNOWLEDGE
                    and step_result.knowledge_query is not None
                    ...
                    payload = self._tool_loop_decision_payload(
                        iteration + 1,
                        "deterministic_knowledge_after_tool",
                    )
                    self.events.record(
                        request.tenant_id,
                        chat_session.id,
    ...
    def _execute_knowledge_query_cycle(
            ...
            try:
                search_response = KnowledgeService(self.db).search(
                    KnowledgeSearchRequest(
                        ...
                        scope=dict(query.scope or {}),
                        mode="chat",
                        knowledge_base_ids=knowledge_base_ids,
                        knowledge_base_version_ids=knowledge_base_version_ids,
                ...
                runtime_plan = self.deterministic_runtime.record_knowledge_result(
                    chat_session,
                    query,
                    {},
                    succeeded=False,
                ...
                if runtime_plan is None:
                    return StepAgentResult(
                        action="reply",
                        reply="流程无法继续（RUNTIME_KNOWLEDGE_RESULT_NOT_RECORDED）。",
                        is_step_completed=False,
        ...
        knowledge_items = {
            ...
            "outcome": search_response.outcome,
            "degraded": search_response.degraded,
            "evidence_sufficiency": search_response.evidence_sufficiency.model_dump(
                mode="json"
            ),
            "knowledge_base_version_ids": knowledge_base_version_ids,
            "selected_buckets": [
        ...
        if DeterministicSopCoordinator.is_enabled(active_skill):
            ...
            deterministic_result = self.deterministic_runtime.merge_plan(
                step_result,
                runtime_plan,
            )
            deterministic_result.knowledge_results = [knowledge_items]
    ...
    def _auto_knowledge_step_result(
        ...
        if selection is not None and selection.degraded:
            self.events.record(
                request.tenant_id,
                chat_session.id,
                "capability_selection_failed",
                {
                    ...
                    or "capability_selection_failed",
        ...
        query_text = (
            (selection.knowledge_query if selection is not None else None)
            or request.message
        ).strip()
        ...
        if knowledge_mode == "auto":
            ...
            self.events.record(
                request.tenant_id,
                chat_session.id,
                "knowledge_prefetch_finished",
                {
                    "phase": "knowledge_prefetch",
                    "mode": "lexical",
                    "outcome": (
                        str(knowledge_items.get("outcome") or "evidence_found")
                        if knowledge_items
                        else "no_match"
                    ),
                    "evidence_count": (
                        len(knowledge_items.get("evidence_pack") or [])
                        if knowledge_items
                        else 0
                    ),
                    "chunk_count": (
                        len(knowledge_items.get("chunks") or [])
                        if knowledge_items
                        else 0
                    ),
                },
    ...
    def _knowledge_items_for_message(
        ...
        search_response = KnowledgeService(self.db).search(
            KnowledgeSearchRequest(
                ...
                scope=dict(knowledge_query.scope or {}),
                mode="chat",
                knowledge_base_ids=knowledge_base_ids,
                knowledge_base_version_ids=knowledge_base_version_ids,
                max_chunks=max(1, min(knowledge_query.max_chunks, 12)),
                max_buckets=4,
                max_depth=max(1, min(knowledge_query.max_depth, 4)),
                need_evidence_pack=True,
            ),
            model_config,
        )
    ...
    def _run_step_agent_with_context_repair(
        ...
        if DeterministicSopCoordinator.is_enabled(active_skill):
            ...
            self.deterministic_runtime.normalize_runtime_slots(
                chat_session,
                active_skill,
            )
        ...
        if DeterministicSopCoordinator.is_enabled(active_skill):
            ...
            model_understanding = step_result.model_copy(
                update={
                    "slot_updates": normalized_updates,
                    "tool_call": None,
                    "next_step_id": None,
                    "is_step_completed": False,
                    "handoff": False,
            ...
            runtime_result = self.deterministic_runtime.prepare_step(
                chat_session,
                active_skill,
                model_understanding,
                user_message=request.message,
            )
            missing_fields, slot_value_contract = (
                self.deterministic_runtime.current_slot_repair_contract(
                    chat_session,
                    active_skill,
                )
            )
            rejected_update_count = len(step_result.slot_updates) - len(normalized_updates)
            if not missing_fields or rejected_update_count <= 0:
            ...
            validation_result = self._run_step_agent_once(
                ...
                repair_context={
                    ...
                    "slot_value_contract": slot_value_contract,
            ...
            repaired_understanding = validation_result.model_copy(
                update={
                    "slot_updates": self.deterministic_runtime.normalize_model_slot_updates(
                        chat_session,
                        active_skill,
                        validation_result.slot_updates,
                    ),
                    "tool_call": None,
                    "next_step_id": None,
                    "is_step_completed": False,
            ...
            if not repaired_understanding.reply and model_understanding.reply:
                repaired_understanding.reply = model_understanding.reply
            self._apply_step_result(
                request.tenant_id,
                chat_session,
            ...
            self.events.record(
                ...
                {
                    "mode": "deterministic_slot_validation",
                    "active_skill_id": chat_session.active_skill_id,
                    "active_step_id": chat_session.active_step_id,
                    "missing_expected_user_info": missing_fields,
                    "slot_value_contract": slot_value_contract,
                    "rejected_update_count": rejected_update_count,
                    "slot_updates": repaired_understanding.slot_updates,
                },
            )
            return self.deterministic_runtime.prepare_step(
                chat_session,
                active_skill,
                repaired_understanding,
                user_message=request.message,
    ...
    def _should_complete_skill(
        ...
        if (
            ...
            and not DeterministicSopCoordinator.is_enabled(skill)
        ):
            return False
        if (
            tool_result
    ...
    def _knowledge_capability_payload(
        self,
        tenant_id: str,
        user_id: str | None,
        agent_id: str | None,
    ) -> dict[str, object]:
        """返回当前成员与数字员工交集内的知识库版本元数据，不包含正文。"""

        knowledge_base_ids, version_ids = self._accessible_knowledge_scope(
            tenant_id,
            user_id,
            agent_id,
        )
        versions = [
            ...
            if (
                (row := self.db.get(KnowledgeBaseVersion, version_id)) is not None
                and row.tenant_id == tenant_id
                and row.knowledge_base_id in knowledge_base_ids
                and row.status == "active"
            )
        ]
        version_by_knowledge_base_id = {
        ...
        cards = [
            ...
            if (row := version_by_knowledge_base_id.get(knowledge_base_id)) is not None
        ]
        return {
            "available": bool(cards),
            "accessible_count": len(cards),
    ...
    def _accessible_knowledge_scope(
        self,
        tenant_id: str,
        user_id: str | None,
        agent_id: str | None,
        *,
        ...
        """返回成员与数字员工知识交集；SOP 调用可追加不含查询正文的审计摘要。"""

        current_user = self.db.get(User, user_id) if user_id else None
        if current_user is None or current_user.tenant_id != tenant_id:
            return [], []
        ...
        versions = accessible_knowledge_base_versions(
            self.db,
        ...
        if audit:
            ...
            for decision in resolution.decisions:
                if not decision.denial_reason:
                    continue
                denial_counts[decision.denial_reason] = (
                    denial_counts.get(decision.denial_reason, 0) + 1
                )
            append_user_management_audit(
                ...
                action_kind="read",
                outcome="success" if versions else "denied",
                resource_type="agent_profile",
                resource_id=agent_id,
                detail={
                    "agent_id": agent_id,
                    "allowed_knowledge_base_count": len(versions),
                    "denial_reason_counts": denial_counts,
                    "source": "deterministic_sop",
                },
            )
        return list(versions), [version.id for version in versions.values()]
    ...
    def _finalize_turn(
        ...
        for metadata_key in (
            ...
            if assistant_metadata.get(metadata_key):
                event_payload[metadata_key] = assistant_metadata[metadata_key]
        self.events.record(
            tenant_id,
            chat_session.id,
            "assistant_message_created",
