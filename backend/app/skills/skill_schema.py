...
@CallChain  : SOP API/编辑器 → SkillCard 请求响应契约 → 发布编译与版本读取
@Description: 定义旧版 SkillCard、编辑请求和带不可变元数据的版本响应契约。
"""
...
SkillCondition = str | dict[str, Any] | None
...
class SkillCard(BaseModel):
    ...
    slot_key_aliases: dict[str, str] = Field(default_factory=dict)
    slot_filling_policy: dict[str, Any] = Field(default_factory=dict)
    response_rules: list[str] = Field(default_factory=list)
    execution_mode: Literal["legacy", "deterministic"] = "legacy"
    condition_schemas: dict[str, dict[str, Any]] = Field(default_factory=dict)
...
class SkillVersionRead(BaseModel):
    ...
    compiled_definition_checksum: Optional[str] = None
    meta_model_version: Optional[int] = None
