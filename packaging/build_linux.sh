...
rm -rf packaging/out/gongge-xuban/runtime
cp -a "$RUNTIME_DL_ROOT/python" packaging/out/gongge-xuban/runtime
...
mkdir -p \
  "$STAGE/DEBIAN" \
  "$STAGE/opt/gongge-xuban" \
  "$STAGE/usr/bin" \
  "$STAGE/usr/share/applications" \
  "$STAGE/usr/share/icons/hicolor/128x128/apps"
cp -a packaging/out/gongge-xuban/. "$STAGE/opt/gongge-xuban/"
cp packaging/assets/gongge-xuban.png "$STAGE/usr/share/icons/hicolor/128x128/apps/gongge-xuban.png"
cat > "$STAGE/usr/bin/gongge-xuban" <<'SH'
#!/bin/sh
exec /opt/gongge-xuban/gongge-xuban "$@"
SH
chmod 0755 "$STAGE/usr/bin/gongge-xuban"
cat > "$STAGE/usr/share/applications/gongge-xuban.desktop" <<'DESK'
[Desktop Entry]
Name=Gongge-Xuban
Comment=Gongge-Xuban desktop service
Exec=gongge-xuban %u
Icon=gongge-xuban
Terminal=false
Type=Application
Categories=Utility;
MimeType=x-scheme-handler/gongge-xuban;
StartupNotify=true
DESK
INSTALLED_SIZE="$(du -sk "$STAGE/opt/gongge-xuban" | cut -f1)"
cat > "$STAGE/DEBIAN/control" <<EOF
Package: gongge-xuban
Version: $DEB_VERSION
Section: utils
Priority: optional
Architecture: amd64
...
Description: Gongge-Xuban desktop service
 Gongge-Xuban runs a local service and opens its interface in the default browser.
EOF
...
DEB_OUT="packaging/out/Gongge-Xuban-linux-x86_64.deb"
dpkg-deb --root-owner-group --build "$STAGE" "$DEB_OUT"

echo "==> [7/8] Building AppImage"
APPDIR="packaging/out/Gongge-Xuban.AppDir"
rm -rf "$APPDIR"
mkdir -p "$APPDIR/usr/bin" "$APPDIR/usr/lib/gongge-xuban"
cp -a packaging/out/gongge-xuban/. "$APPDIR/usr/lib/gongge-xuban/"
cat > "$APPDIR/usr/bin/gongge-xuban" <<'SH'
#!/bin/sh
HERE="$(dirname "$(readlink -f "$0")")"
exec "$HERE/../lib/gongge-xuban/gongge-xuban" "$@"
SH
chmod 0755 "$APPDIR/usr/bin/gongge-xuban"
cat > "$APPDIR/AppRun" <<'SH'
#!/bin/sh
HERE="$(dirname "$(readlink -f "$0")")"
exec "$HERE/usr/bin/gongge-xuban" "$@"
SH
chmod 0755 "$APPDIR/AppRun"
cp "$STAGE/usr/share/applications/gongge-xuban.desktop" "$APPDIR/gongge-xuban.desktop"
cp packaging/assets/gongge-xuban.png "$APPDIR/gongge-xuban.png"
...
APPIMAGE_OUT="packaging/out/Gongge-Xuban-linux-x86_64.AppImage"
